...
# Output: Intensity ratio function value to right(external) side with j fixed
#
...
for i in range(n):
    for j in range(m):
        soma1 = sum(np.log(x[i]  + z[nli:nlf]))
        soma2 = sum(np.log((x[i] + z[nli:nlf])**2 - 4 * x[i] * abs(y[j])**2 * z[nli: nlf]))
        aux1 = L * np.log(x[i])
        aux2 = L * np.log(1 - abs(y[j])**2)
        aux3 =                       soma1 / (dim - pix)
        aux4 = (0.5 * (2 * L + 1)) * soma2 / (dim - pix)
