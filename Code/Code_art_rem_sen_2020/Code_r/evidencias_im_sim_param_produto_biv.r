...
source("loglike_produto_biv.r")
source("loglikd_produto_biv.r")
# Programa principal
setwd("../..")
setwd("Data")
# canais hh, hv, and vv
...
for (k in 150 : 150){# k aqui varre o número de radiais
  ...
  for (j in 1 : (N - 1)){
    ...
    r2 <- sum(z2[1: j]) / j
    r3 <- 0.1
    res1 <- maxBFGS(loglike_produto_biv, start=c(r1, r2, r3))
    r1 <- sum(z1[(j + 1): N]) / (N - j)
    r2 <- sum(z2[(j + 1): N]) / (N - j)
    r3 <- 0.1
    res2 <- maxBFGS(loglikd_produto_biv, start=c(r1, r2, r3))
