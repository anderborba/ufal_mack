function [phantom] = Generate_PolSAR_two_classes_phantom_rectangle_with_ramp_neg(S, L, m, n, img1, rows1, cols1, img2, rows2, cols2, img3, rows3, cols3)
...
% Outside of strip
aux3(1:m,1:n,1) = reshape(S_class_1(1,1,:),m,n);
aux3(1:m,1:n,2) = reshape(S_class_1(2,2,:),m,n);
aux3(1:m,1:n,3) = reshape(S_class_1(3,3,:),m,n);
aux3(1:m,1:n,4) = reshape(real(S_class_1(1,2,:)),m,n);
aux3(1:m,1:n,5) = reshape(imag(S_class_1(1,2,:)),m,n);
aux3(1:m,1:n,6) = reshape(real(S_class_1(1,3,:)),m,n);
aux3(1:m,1:n,7) = reshape(imag(S_class_1(1,3,:)),m,n);
aux3(1:m,1:n,8) = reshape(real(S_class_1(2,3,:)),m,n);
aux3(1:m,1:n,9) = reshape(imag(S_class_1(2,3,:)),m,n);
L1 = length(rows3);
L2 = length(cols3);
for i= 1: L1
	phantom(rows3(i), cols3(i),1) = aux3(rows3(i), cols3(i),1) + img3(rows3(i), cols3(i));
	phantom(rows3(i), cols3(i),2) = aux3(rows3(i), cols3(i),2) + img3(rows3(i), cols3(i));
	phantom(rows3(i), cols3(i),3) = aux3(rows3(i), cols3(i),3) + img3(rows3(i), cols3(i));
	phantom(rows3(i), cols3(i),4) = aux3(rows3(i), cols3(i),4) + img3(rows3(i), cols3(i));
	phantom(rows3(i), cols3(i),5) = aux3(rows3(i), cols3(i),5) + img3(rows3(i), cols3(i));
	phantom(rows3(i), cols3(i),6) = aux3(rows3(i), cols3(i),6) + img3(rows3(i), cols3(i));
	phantom(rows3(i), cols3(i),7) = aux3(rows3(i), cols3(i),7) + img3(rows3(i), cols3(i));
	phantom(rows3(i), cols3(i),8) = aux3(rows3(i), cols3(i),8) + img3(rows3(i), cols3(i));
	phantom(rows3(i), cols3(i),9) = aux3(rows3(i), cols3(i),9) + img3(rows3(i), cols3(i));
end
