function [bw, bw_strip, bw_entire] = gera_step_0_1(N, xl, xu, yl, yu, eps)
bw = ones(N);
bw_strip = ones(N);
bw_entire = ones(N);
...
for i= xl - eps: xl + eps
	for j =  yl + eps: yu - eps
		...
		bw_entire(i, j)= 1.0 - fun_smooth_ramp(xaux) ;
	end
end
...
for i= xu - eps: xu + eps
	for j =  yl + eps: yu - eps
		...
		bw_entire(i, j)= fun_smooth_ramp(xaux);
	end
end
...
for i= xl + eps: xu - eps
	for j =  yl - eps: yl + eps
		...
		bw_entire(i, j)= 1.0 - fun_smooth_ramp(yaux);
	end
end
...
for i= xl + eps: xu - eps
	for j =  yu - eps: yu + eps
		...
		bw_entire(i, j)= fun_smooth_ramp(yaux);
	end
end
...
for i= xl - eps: xl + eps
	for j= yl - eps: yl + eps
		xaux = (i - (xl - eps)) / (2.0 * eps);
		yaux = (j - (yl - eps)) / (2.0 * eps);
		bw_strip(i, j)= 1.0 - fun_smooth_ramp_3d_corner(xaux, yaux);
		bw_entire(i, j)= 1.0 - fun_smooth_ramp_3d_corner(xaux, yaux);
	end
end
