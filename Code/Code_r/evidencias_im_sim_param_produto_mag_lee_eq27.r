...
require(maxLik)
#
source("func_obj_l_gauss_lee_eq27.r")
source("func_obj_l_gauss_copia1.r")
source("loglike_prod_mag_lee_eq_27.r")
source("loglikd_prod_mag_lee_eq_27.r")
# Programa principal
setwd("../..")
setwd("Data")
# canais hh, hv, and vv
...
for (k in 1 : 1){
  ...
  for (j in 1 : N){
    ...
    #                random.seed = 123)
    res1 <- maxBFGS(loglike_prod_mag_lee_eq_27, start=c(r1, r2))
    ...
    #                parscale = rep(1, length = length(start)),
    #                random.seed = 123)
    res2 <- maxBFGS(loglikd_prod_mag_lee_eq_27, start=c(r1, r2))
  ...
  out   <- GenSA(lower = lower, upper = upper, fn = func_obj_l_gauss_lee_eq27, control=list(maxit = 100))
...
for (j in 1 : (N - 1) ){
  ...
  lobj[j] <- func_obj_l_gauss_lee_eq27(j)
}
...
#setwd("../..")
#setwd("Data")
#sink("evid_real_flevoland_produto_mag_param_L_rho_1_2.txt")
#print(dfev)
#sink()
#setwd("..")
#setwd("Code/Code_r")
