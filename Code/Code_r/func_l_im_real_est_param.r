...
pp <- ggplot(dfp, aes(x = xp, y = z, color = 'darkred')) + geom_line() + xlab(TeX('Pixels na faixa de dados ')) + ylab(TeX('$f_z(j)$')) + ggtitle(TeX('Distribuiçcao de dados na faixa')) + guides(color=guide_legend(title=NULL)) + scale_color_discrete(labels= lapply(sprintf('$\\sigma_{hh} = %2.0f$', NULL), TeX))
