...
df <- data.frame(x = xx, y1 = freq_hh, y2 = freq_hv, y3 = freq_vv)
alpha <- c(1,2,3)
p <- ggplot(df) 
...
pp <- p + geom_line(aes(x = x, y = y1, color = "Ihh"))+  geom_line(aes(x = x, y = y2, color = "Ihv"))+  geom_line(aes(x = x, y = y3, color = "Ivv")) + scale_y_log10(limits = c(0.002, 1)) + ylab(TeX('Probability')) + xlab(TeX('Detection error')) 
print(pp) 
setwd("../..")
setwd("Text/Dissertacao/figuras")
ggsave("metricas_ihh_ivh_ivv_nhfc_artigos.pdf")
setwd("../../..")
setwd("Code/Code_r")
