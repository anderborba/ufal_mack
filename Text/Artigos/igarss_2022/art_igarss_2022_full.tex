...
$$f(x,y)=
  \begin{cases}
    0                 & \textrm{if } x,y < 0 \\
    f_x(x)  f_y(y) & \textrm{if } 0 \leq x,y \leq 1 \\
    1                 & \textrm{if } x,y > 1.
  \end{cases}$$

The idea to built the simulated image was based in \cite{gamf} and the parameters $\mu_i$ also.

With base in the figure above and \cite{gamf}, we define the samples distributed as Wishart, where $L$, $\mu_1$, $\mu_2$, and $\mu_{av}= {(\mu_1+\mu_2)}/{2}$ in all channels. 
We use $\mu_{av}$ to strip in the image.
