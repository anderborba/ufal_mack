...
\label{sec:intro}
Natural accidents such as earthquakes or flooding have great destructive power. The consequences can be from an economic point of view as well as the loss of human life. Highlighting the second case's importance, the faster the emergency response, we can save the most significant number of human lives. Natural accidents are difficult to predict with current knowledge and technology.

Problems such as the collapse of debris or rapid flooding of areas cause severe damage requiring quick rescues to high-risk regions. Thus, the remote sensing area assists in the natural accident's emergency response requirement because fast and efficient information is crucial for successful savings. This implies that the remote sensing area has increased in importance in investigating information about disasters. 
...
The automatic detection of risk constructions or the flood's evolution has practical meaning or values for the society. For more information see \cite{zhp}, \cite{zshp} and \cite{czwz}.

To assist in the risk alarm process and speed up people's rescue in regions that have suffered natural accidents, the edge detection method is essential. Thus, this work aims to quantify the influence of each intensity channel in the process of edge detection, with information fusion being an extension of \cite{bmf_2020}.
...
Fig \ref{fig:fusion_flowchart} shows the process from getting evidence edge until fusion information. To more details see \cite{bmf_2020}.
\pgfdeclarelayer{background}
\pgfdeclarelayer{foreground}
\pgfsetlayers{background,main,foreground}
%
...
\caption{Flowchart to evidence fusion process}
\label{fig:fusion_flowchart}
\end{figure}
%
\section{Metrics}\label{sec:metrics}
This section is based on the references \cite{chicco}, and \cite{btr} and it presents metrics used to measure the efficiency of methods edge detection evidence and their information fusion. The metrics are based on the confusion matrix of the binary images generated compared to the Ground Reference image (GR). We consider the positive instance by p (pixel value 1) and the negative instance by n (pixel value 0).

The instances can be labeled and classified by: TP is positive labeled and correctly classified as positive, FN is positive labeled and wrongly classified as negative, TN is negative labeled and correctly classified as negative, and FP is negative labeled and wrongly classified as positive. 
...
Table \ref{metricas_erro} shows the metrics used,
\begin{table}[hbt]
	\centering
	\caption{Formulae to metrics.}\label{metricas_erro}
	\begin{tabular}{@{}ll@{}}\toprule
		Metrics               & Formulae       \\ \midrule\vspace{0.4cm}
	...
	\end{tabular}
\end{table}
where the metric Matthews correlation coefficient is defined by,
\begin{equation*}
	\text{Mcc}=\frac{\text{TP}\cdot\text{TN}-\text{FP}\cdot\text{FN}}{\sqrt{\text{(TP +FP)}\cdot\text{(TP +FN)}\cdot\text{(TN +FP)}\cdot\text{(TN +FN)}}}
\end{equation*}
and, all metrics have in the best case is 1 (100\%), and in the worst case is 0 (0\%). The symbol nMcc means the normalized metric Mcc. 
...
To the metric accuracy, the methods showed excellent performance; the worst measure happened in the MR--SWT fusion method and has a value of 99.33\%. The performance of the method to find edges evidences in the intensities channels showed excellent performance. The use of this metric, in this test case, is inconclusive to order the channels in a degree of importance in the fusion of information. Realizing the visual inspection, we verified that the fusion using the ROC statistics presents sparsity. However, the excellent performance of this method is because the technique detects few FP and FN.

For the case of the MFe metric, the methods present low performance. The worst case is the merger method based on the ROC statistic, whose metric value is 1.39\%, and the best approach is MR--SWT with a performance of 11.65\% higher. The Mfe metric can provide an ordering of the importance of each channel in the fusion of information; for example, we can attribute to the channel $I_\text{hh}$ the best performance of edge detection with the MLE method, which agrees with the results obtained in \cite{bmf_2020}.  We can extract from this metric the best performance of the fusion methods concerning the methods of detecting evidence of edges in each channel, thus justifying the use of methods with information fusion. Analyzing Mfe's formula, we can notice that the low performance is the high values for FP and FN compared to TP; besides, the metric does not show information about the undetected edges because it does not take into account the importance of TN.

Considering the nMcc metric, the worst performance is observed for the metric based on the ROC statistic fusion at 51.19\%, and the better result was recorded for the MR--DWT method, 22,41 \% higher. As with the Mfe metric results, the nMCC metric can rank the intensity channels in order of importance to perform the fusion of information. The two metrics show results similar to those obtained in \cite{bmf_2020}. 

We can state that the metric accuracy does not provide information on the use or disposal of intensity channels in the fusion process. At the same time, Mfe and nMcc metrics have the characteristic of ordering the channels of intensities to be used or discarded in the fusion process. Therefore we can quantify the influence of intensity channels on the process of fusing information for edge detection in PolSAR images.
%
%-------------------------------------------------------------------------
%\vfill
%\pagebreak
