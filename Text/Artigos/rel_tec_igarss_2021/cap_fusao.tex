...
A imagem binária $\widehat{\bm\jmath}_\text{hv}$ ilustrada na figura \ref{fig:bin_evidencias_hv} é o resultado da aplicação do método de detecção de evidências de bordas no canal hv da imagem de Flevoland descrita no capítulo  \ref{cap_resultados}. 
\begin{figure}[hbt!]
\centering
\includegraphics[width=4.0in]{flevoland_hv_evid_crop_teste}
	\caption{Imagem binária das evidências de bordas .}
\label{fig:bin_evidencias_hv}
\end{figure}
%%% ACF Você abusa da voz passiva, que torna o texto cansativo
%%% AAB Vou prestar mais atencao

Neste trabalho são propostas seis técnicas de fusão de evidências de bordas:
\begin{itemize} 
\item Média simples; 
...
O fluxograma do procedimento de fusão pode ser observada na figura~\ref{fig:fluxo_fusao},
...
\label{fig:fluxo_fusao}
\end{figure}
onde, GR (\textit{Ground Reference}) é a imagem de referência gerada apontando as bordas corretamente. Este tipo de imagem é usado para calcular o erro dos métodos. 

Cada uma das técnicas serão descritas nesta seção.
...
Para cada imagem binaria $\bm{\widehat\jmath}_c$ foram aplicados filtros DWT, sendo um filtro passa baixa \textbf{L}  na direção vertical e um filtro passa alta \textbf{H} na direção horizontal, ambos são \textit{down-sampled} para formar as matrizes de coeficientes $\bm{\widehat\jmath}_{c\text{L}}$ e $\bm{\widehat\jmath}_{c\text{H}}$.  
...
O método PCA é baseado nos trabalhos de \citet{n_r,mit}, e pode ser resumido nos seguinte passos:
\begin{enumerate}
...
\item Calcular as matrizes de autovalores ($\bm\Lambda$) e autovetores ($\bm V$) da matriz de covariância, ordenar os autovalores e seus respectivos autovetores de maneira decrescente;
...
\begin{enumerate}
\item Somar as imagens binárias $\bm{\widehat\jmath}_c$ para produzir a matriz de frequência V; 
...
Na matriz de confusão a soma de todas as possibilidades dos resultados em uma classificação retorna o valor TP+FN+FP+TN=1, quando os valores de TP, FN, FP e TN são expressos em termos de probabilidade..
...
\end{equation}
 ...
 Na métrica conhecida por razão de positivos verdadeiros, taxa de acerto, \textit{recall} ou  sensibilidade,~$tp_{\text{rate}}=\frac{\text{TP}}{\text{P}}=\frac{\text{TP}}{\text{TP+FN}}$,~se o número de negativos falsos tender a zero, o valor da métrica aproxima-se do valor unitário. Assim como, na métrica conhecida como razão de positivos falsos,~$fp_{\text{rate}}=\frac{\text{FP}}{\text{N}}=\frac{\text{FP}}{\text{FP+TN}}$,~se o número de verdadeiros positivos tender a zero, o valor da métrica aproxima-se do valor unitário.
...
Resumindo, somando-se as matrizes $\bm{\widehat\jmath}_c$ temos a matriz de frequência $\text{V}=\sum_{c=1}^{n_c}\bm{\widehat\jmath}_c$. Na matriz V é aplicado os limiares $\text{CT}_t$ com $t=1,\dots,n_c$, gerando os mapas de evidências de bordas $\bm{\widehat\imath}_t$. A figura \ref{fig:cap_fusao_roc1} mostra o fluxograma correspondente a esse processo.
\pgfdeclarelayer{background}
\pgfdeclarelayer{foreground}
\pgfsetlayers{background,main,foreground}
\tikzstyle{sensor}=[draw, fill=blue!20, text width=5em, 
...
Para cada matriz $\bm{\widehat\imath}_t$, com $t$ fixado arbitrariamente, foi feita uma comparação com todas as matrizes $\bm{\widehat\jmath}_c$.  e construído $n_c$ matrizes de confusão. Uma delas é mostrada na Tabela \ref{tab:matriz_conf_roc_j}.
\begin{table}[htb!]
	\centering
...
O gráfico para a curva ROC é bidimensional, os valores das razões de positivos falsos, $\text{FP}_{rate_t}$, são medidos no eixo horizontal, e as razões de positivos verdadeiros, $\text{TP}_{rate_t}$, são medidos no eixo vertical. Assim, cada mapa de borda $\bm{\widehat\imath}_t$ produz um ponto no gráfico $(\text{FP}_{rate_t}, \text{TP}_{rate_t})$ gerando a curva ROC. 

O trabalho de \cite{fawcett} mostra que o limiar ótimo ocorre na intersecção, ou perto da mesma, da curva ROC com a linha diagnóstico, que é formada conectando os pontos (P,P) e (0,1) representada pela linha vermelha na figura \ref{fig:curva_roc}. 
...
Na figura \ref{fig:curva_roc} podemos ver os limiares encontrados com o método de fusão E-ROC dos canais hh, hv, e vv, aplicado a imagem simulada descrita no próximo capítulo. A linha verde representa as distâncias euclidianas d1, d2, e d3 entre os pontos encontrados $(\text{FP}_{rate_t}, \text{TP}_{rate_t})$ até a reta diagnóstico em vermelho.

A análise da fusão E-ROC para a imagem simulada definiu a reta d2 como sendo a de menor distância, portanto o limiar ótimo é $t=2$, e a imagem $\bm{\widehat\imath}_2$ tem o melhor desempenho na tarefa de detectar bordas. 
%%% ACF seria interessante desenhar os segmentos representativos das distâncias dos pontos à linha diagnóstico
%%% AAB realizado

\begin{figure}[hbt]
\centering
\includegraphics[width=4.0in]{curva_roc_3_canais_com_distancia.pdf}
...
\item Recomeçar o procedimento em (1) para $\bm\Phi_r$ com $r=2$, até o menor nível de resolução $R$;
\item A decomposição MR-SVD em cada canal é  
\begin{equation}\nonumber
...
A figura \ref{fig:cap_fusao_mrsvd} mostra o fluxograma correspondente a esse processo.
\pgfdeclarelayer{background}
\pgfdeclarelayer{foreground}
\pgfsetlayers{background,main,foreground}
\tikzstyle{sensor}=[draw, fill=blue!20, text width=5em, 
