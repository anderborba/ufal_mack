...
\chapter{Introdução}
\label{cap_acf}
Os radares de abertura sintética (SAR -- \textit{Synthetic Aperture Radar}) e polarimétricos de abertura sintética (PolSAR) alcançaram uma posição essencial na área de sensoriamento remoto. 
...
Como vantagens podemos destacar que a captação das imagens pode ser realizadas dia e noite (pois o sensor é ativo e carrega a própria fonte de iluminação), não dependem das condições do tempo (pois iluminam a cena com micro-ondas, que são pouco afetadas por nuvens, chuva, bruma, fumaça, etc.) e possuem  excelente capacidade de penetração da cobertura vegetal. 
A desvantagem deste tipo de imagem está na presença do ruído \textit{speckle}, que gera um aspecto granular na imagem, dificultando a sua interpretação e análise. Os livros \citet{lp} e \citet{lkc} são excelentes fontes para consultar sobre imagens PolSAR e Sensoriamento Remoto.
\index{Sensoriamento Remoto}
\index{PolSAR}
...
e técnicas estatísticas \citet{gmbf,gmbf_sc, fbgm, nhfc}.
\index{Speckle}
\index{Markov}
\index{Deep learning}

Este trabalho de pesquisa segue a abordagem de modelagem estatística usando as técnicas descritas em \citet{gmbf, gmbf_sc, fbgm, nhfc} para encontrar evidências de bordas em imagens PolSAR. 
...
%%% AAB Realizado  
O artigo \citet{bmf_2020} fornece informações sobre os processos de fusão em imagens PolSAR, assim como o  desempenho de cada método para detectar bordas, enquanto o livro \citet{mit} mostra os fundamentos da fusão de imagens.


O Algoritmo Gambini, apresentado em \citet{gmbf, gmbf_sc}, é uma excelente téc\-ni\-ca para detecção de bordas. 
Com essa técnica é possível encontrar evidências de uma borda sobre uma faixa fina de dados, ou seja, informações obtidas em uma faixa de dados da imagem de aproximadamente um píxel.
Esta abordagem funciona com qualquer modelo, o que o torna adequado para dados SAR e PolSAR, e tem demostrando bom desempenho para detectar evidências de bordas. 
Esse algoritmo consiste em traçar raios do centroide de uma região de interesse até um ponto externo da região, para então, encontrar a evidência de borda maximizando uma função que, na proposta desta tese, é a de máxima verossimilhança total.
...
Para não perder generalidade, optou-se pela distribuição Wishart para as observações totalmente polarimétricas, derivando para as suas distribuições, do tipo gama, em cada canal de intensidade. 
A função de máxima verossimilhança total depende das estimativas que parametrizam as leis gama. 
As estimativas são realizadas pelo método de otimização BFGS
\citep[Broyden-Fletcher-Goldfarb-Shanno, ver][]{ht,nw}. 
Neste trabalho estimaremos os parâmetros que indexam as distribuições gama com o método de máxima verossimilhança (MLE -- \textit{Maximum Likelihood Estimator}).
...
Estatísticas baseadas na curva ROC \citep[\textit{Receiver Operating Characteristic},][]{gs}, o qual será denominado de E--ROC; 
...
\index{MR--SVD}
\section{Objetivos}
O objetivo dese trabalho é estudar a viabilidade de métodos para a fusão das evi\-dên\-cias de bordas detectadas nos canais de intensidades hh, hv, e vv, de uma imagem PolSAR, como também usar métodos de fusão para quantificar as contribuições de informações provenientes de cada canal. A realização da fusão é importante, pois possibilita a decisão de usar ou descartar as informações de um dado canal para melhorar o desempenho da detecção de bordas. Com a informação da influência de cada canal na fusão podemos decidir quais canais da imagem PolSAR serão adquiridos. 

Para alcançar tais objetivos vamos aplicar nas imagem SAR e PolSAR os seguintes procedimentos:
\begin{enumerate}[label=(\roman*)]
	\item \label{item:obj_i}  Especificar manualmente ou automaticamente a região de interesse ROI;\index{ROI}
	\item \label{item:obj_ii} Em cada ROI calcular o centro de massa e, a partir desse ponto, traçar retas radiais cruzando duas amostras distintas. O artigo \citet{gmbf} propõe um método para garantir que cruzamos duas amostras. 
...
%%% AAB Realizado
\section{Plataformas, validação dos programas, recursos computacionais}
...
O recurso computacional disponível para esta pesquisa consiste em um computador Intel\copyright\ Core i7-9750HQ CPU \SI{2.6}{\giga\hertz} com \SI{16}{\giga\byte} de memória RAM.

\section{Reprodutibilidade e replicabilidade}
\index{Reprodutibilidade}\index{Replicabilidade}
A reprodutibilidade e replicabilidade são essenciais para o desenvolvimento da ciên\-cia, estes temas não são discutidos neste trabalho, porém realizamos as pesquisas de forma a respeitar esses conceitos. Para maiores informações consulte os artigos de \citet{br} e \citet{fgmed}.

% mostram pesquisas sobre o tema. 
%No primeiro artigo encontramos as seguintes definições: 
%\begin{itemize}
...
%%% AAB Realizado
\item  \label{item:org_text_i} Capítulo \ref{cap_asp_gerais}: Alguns conceitos introdutórios e informações relevantes sobre as imagens SAR, PolSAR e radares. 
