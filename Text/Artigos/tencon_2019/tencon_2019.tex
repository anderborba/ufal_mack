...
\begin{itemize}
\item[-] calculate the SWT decomposition by getting $L_\text{HH}$, $L_\text{HL}$, $L_\text{LH}$ and $L_\text{LL}$ for each channel; %%% ACF O que é cada uma dessas L?
...
In this article, we analyzed the methods of fusion of edges evidence in Polsar images. First, we found the edges evidence using the method of maximum likelihood in the intensities channels. Secondly, fusion methods simple average, SWT, PCA, and ROC curve were applied. 

To quantify and compare the performance of the edge detection process, we use a simulated image. In this image, we apply the method to get the $\ell{\jmath}$ function, which has the characteristic of not being differential in many places. Therefore, we recognize the difficulties of using classical optimization methods. To solve the problem, we apply the Simulated Annealing approach because it works correctly in non-differentiable functions.

 The metric, based on the probability of detecting the edge, was applied to the proposed method by showing excellent performance in detecting evidence of edges in the intensity channels.
