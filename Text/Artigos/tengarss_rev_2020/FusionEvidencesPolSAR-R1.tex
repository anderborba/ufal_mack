...
ROC statistics~\cite{gs,fawcett},
...
\For{each band $1\leq c\leq n_c$}{
	\For{each ray passing through the centroid}{
		...
		\For{each $\min_s\leq j\leq \max_s$}{\nllabel{Line:InitFor}
			Partition the sample as $\bm z_{\text{I}}=(z_{\min_s},\dots,z_j)$ and 
			$\bm z_{\text{E}}=(z_{j+1},\dots,z_{\max_s})$; where $\max_s=n-\min_s$\;
...
In our implementation, we replace the exhaustive sequential search (the innermost \textbf{for} loop) by Simulated Annealing.
