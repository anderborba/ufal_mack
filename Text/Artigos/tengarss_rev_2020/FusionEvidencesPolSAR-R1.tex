...
$L$ is the number of looks, $\traco(\cdot)$ is the trace operator of a matrix, $\Gamma_m(L)$ is the multivariate Gamma function defined by $
...
Denoting $H(k)$ the number of lines for which the error is less than $k$ pixels, an estimate of this probability is $f(k)={H(k)}/{n_r}$, where $n_r$ is number of radii. 
...
Such diversity of information content enhances the need of fusing the edge evidences.
...
We quantitatively assessed the result by checking the closeness of the fused points to the actual edge, and by the presence of outliers.
