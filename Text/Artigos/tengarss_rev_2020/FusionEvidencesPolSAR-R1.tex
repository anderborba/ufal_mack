...
\For{each band $1\leq c\leq n_c$}{
	\For{each ray passing through the centroid}{
		...
		$\widehat\jmath\leftarrow$ the value of $j$ at which the total log-likelihood function is maximized\;
...
Notice that the latter (Fig.~\ref{fusion_flev_met}\subref{fusion_flev_met:c}) eliminates the wrong detection close to the center of the area, and has fewer wrongly detected points outside the region of interest.
