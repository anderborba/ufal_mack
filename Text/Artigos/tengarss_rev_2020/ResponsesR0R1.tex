...
We have inserted two more cases, the first considering another region of interest in Flevoland's image. Figures 4(a) and (b) show the region of interest and figure (5) the detection of edge evidence for each channel. The edge evidences fusion are shown in figure (6) for the methods that showed the best performance.
For the second region using the San Francisco bay image, we show the region of interest in figure (7) and the edge detection for each channel in figure (8). The edge evidences fusion are shown in figure (8) for the methods that showed the best performance.    

By inserting the tests, we improve the conclusion of the article. 
\end{tcolorbox}
