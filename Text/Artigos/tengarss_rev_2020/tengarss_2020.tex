...
The \text{BFGS}~(Ref.\cite{nw}) optimization method implemented in the \text{maxLik}~(Ref.\cite{ht}) package was used to estimate the $(\widehat{\mu}_I, \widehat{L}_I)$ and $(\widehat{\mu}_E, \widehat{L}_E)$ parameters, for every $j$ in the data strip.
...
To estimate the parameter $\widehat{\jmath}$ in the following optimization problem $\widehat{\jmath}= \arg\max\limits_{j\in [1,N]}\ell(j;\widehat{\mu}_I, \widehat{L}_I,\widehat{\mu}_E, \widehat{L}_E)$, was used the \text{GenSA} - Generalized Simulated Annealing methods~(Ref.\cite{xgsh}).
