...
\item The result of the fusion $\text{I}_F$ is the inverse DWT transformation.
\end{enumerate}
...
\begin{enumerate}
\item organize the data in such a way that $\bm{\widehat\jmath}_c$ is a column vector, forming the matrix $\text Y$ of dimension $\ell\times n_c$;
...
\item find $C$, the covariance matrix of $\text X$, of dimension $nc\times nc$;
