...
\item Fuse $\text{I}_F(x,y)=\sum_{c=1}^{n_c}\text{P}_c\bm{\widehat\jmath}_c(x,y)$.
\end{enumerate}
...
\item The fusion $I_F$ is the matrix $\text{M}_t$ which corresponds to the optimal threshold.
\end{enumerate}
...
\item Organize the binary image $\bm{\widehat\jmath}_c$ as non-overlapping $2\times 2$ blocks, and arrange each block as a $4\times 1$ vector by stacking columns to form the data matrix $\text{X}_\ell$.
...
\item Find the SVD decomposition $\text{T}=\text{X}_\ell \text{X}_\ell^\text{T}=\text{U}_\ell \text{S}^2 \text{U}_\ell^T$;
%where $s(1)\geq s(2) \geq s(3) \geq s(4)$ compose the singular value matrix S, 
%%% ACF Não definiu nem usou s(1), s(2) etc
$\text U$ is unitary.
\item 
Transform the lines of $\widehat{\text{X}}_\ell=\text{U}_\ell^T\text{X}_\ell$ into new matrices with dimensions $\frac{\text{M}}{2}\times\frac{\text{N}}{2}$: $\{\Phi_\ell, \Psi_\ell^\text{V}, \Psi_\ell^\text{H}, \Psi_\ell^\text{D}\}$. 
\item Repeat the procedure on $\Phi_\ell$ $L-1$ times. 
...
\item The fusion $\text{I}_F(x,y)$ is the SVD transformation to $\overline{\Phi}_L$, $\overline{\text{U}}_\ell$, $\overline{\Psi}_\ell^\text{V}$, $\overline{\Psi}_\ell^\text{H}$ and $\overline{\Psi}_\ell^\text{D}$,  for each level $\ell=1,\dots, L$. 
\end{enumerate}
