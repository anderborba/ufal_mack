...
But images coming from PolSAR sensors have speckle noise, making their processing and analysis challenging tasks. 
The present study discusses a detection method based on the fusion of evidences obtained in the intensity (hh), (hv), and (vv) channels of PolSAR multi-look images. 
The method consists of detecting transition points in the thinnest strip of data that covers two regions using the maximum likelihood under the Wishart model. 
The fusion methods used are 
the simple average, 
the multi-resolution discrete wavelet transform (MR-DWT), 
the multi-resolution stationary wavelet transform (MR-SWT), 
principal component analysis (PCA), 
ROC statistics, and 
a multi-resolution method based on singular value decomposition (MR-SVD). 
The results indicate an assessment of the influence of each intensity channel in the fusion of evidences, with possible paths for future research.
\end{abstract}
...
We show the feasibility of these procedures for edge detection in PolSAR images. 
...
$\text{I}_\text{F}(x,y)=(n_c)^{-1}\sum_{c=1}^{n_c} \widehat{\bm\jmath}_c(x,y)$.
...
\item The result of the fusion $\text{I}_\text{F}$ is the inverse DWT transformation of the coefficient matrices $\bm{\bar\jmath}_{c\text{HH}}$, $\bm{\bar\jmath}_{c\text{LL}}$, $\bm{\bar\jmath}_{c\text{LH}}$, and $\bm{\bar\jmath}_{c\text{HL}}$.
\end{enumerate}
...
\begin{enumerate}
\item Stack the binary images $\bm{\widehat\jmath}_c$ in column vectors to obtain the matrix $\text{X}_{\ell\times n_c}$.
...
\item Compute the matrices of eigenvalues ($\Lambda$) and eigenvectors ($\text V$) of the covariance matrix, sorted in decreasing order by the eigenvalues. %The matrices generated by the eigenvalues, on the principal diagonal, and the eigenvectors placed in columns;
%%% ACF O que é V_c?
\item Compute the components $\text{P}_c=({\sum_{m=1}^{n_c} \text{V}_m})^{-1}{\text{V}_c}$, where $\text{V}_c$ is a column of $\text V$; notice that $\sum_{c=1}^{n_c}\text{P}_c=1$.
\item Fuse $\text{I}_\text{F}(x,y)=\sum_{c=1}^{n_c}\text{P}_c\bm{\widehat\jmath}_c(x,y)$.
\end{enumerate}
...
\item Add the binary images $\bm{\widehat\jmath}_c$ to produce the frequency matrix ($\text V$).
\item Use thresholds ranging from $t=1,\dots,n_c$ on $\text V$ to generate matrices $\text{M}_t$.
\item Compare each $\text{M}_t$ with all $\bm{\widehat\jmath}_c$, find the confusion matrix to generate the ROC curve. 
...
\item The fusion $\text{I}_\text{F}$ is the matrix $\text{M}_t$ which corresponds to the optimal threshold.
\end{enumerate}
...
\item Organize the binary image $\bm{\widehat\jmath}_c$ as non-overlapping $2\times 2$ blocks, and arrange each block as a $4\times 1$ vector by stacking columns to form the data matrix $\text{X}_1$.
...
\item Find $\text{T}_1=\text{X}_1 \text{X}_1^\text{T}=\text{U}_1 \text{S}_1^2 \text{U}_1^\text{T}$, the SVD decomposition of $\text{X}_1$, where $\text U_1$ is unitary.
\item Calculate $s(1)\geq s(2) \geq s(3) \geq s(4)$ which comprise the singular value matrix $\text{S}_1$.
%%% ACF Não definiu nem usou s(1), s(2) etc
%%% AAB Os s(1), s(2), s(3) e s(4) são usados para gerar a matriz $S$

\item 
Transform the lines of $\widehat{\text{X}}_1=\text{U}_1^\text{T}\text{X}_1$ into new matrices with dimensions $\frac{m}{2}\times\frac{n}{2}$: $\{\Phi_1, \Psi_1^\text{V}, \Psi_1^\text{H}, \Psi_1^\text{D}\}$. 
%\item Repeat the procedure on $\Phi_r$ $R$ times. Where $r=1,\dots,\text{R}$ and, R is lowest resolution index. 
\item Repeat the procedure on $\Phi_r$. Where $r=1,\dots,R$, with $R$ the lowest resolution index. 
\item The MR-SVD method is represented as
\begin{equation}\label{msvd_iter}
\widehat{\text{X}}\leftarrow \left\{\Phi_R,\{\Psi_r^\text{V},\Psi_r^\text{H},\Psi_r^\text{D} \}_{r=1}^R,\{\text{U}_r\}_{r=1}^R \right\}.
\end{equation}
...
\item Compute $\overline{\Phi}_R$, the average of all $\Phi_r$; and $\overline{\text{U}}_r$, the average of all $\text{U}_r$.
\item Find the pixel-wise maximum of $\Psi_r^\text{V}$, $\Psi_r^\text{H}$ and $\Psi_r^\text{D}$ : $\overline{\Psi}_r^\text{V}$, $\overline{\Psi}_r^\text{H}$ and $\overline{\Psi}_r^\text{D}$ .
\item The fusion $\text{I}_\text{F}(x,y)$ is the SVD transformation of $\overline{\Phi}_R$, $\overline{\text{U}}_r$, $\overline{\Psi}_r^\text{V}$, $\overline{\Psi}_r^\text{H}$ and $\overline{\Psi}_r^\text{D}$,  for each level $r=1,\dots,\text{R}$. 
\end{enumerate}
