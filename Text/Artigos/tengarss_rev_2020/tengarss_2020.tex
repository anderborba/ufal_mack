...
\item Organize the binary image $\bm{\widehat\jmath}_c$ as non-overlapping $2\times 2$ blocks, and arrange each block as a $4\times 1$ vector by stacking columns to form the data matrix $\bm X_1$ with dimension ${4\times\frac{\ell}{4}}$.
...
\item Find the SVD decomposition
 of $\bm X_1=\bm U_1 \bm S_1 \bm V_1^T$, where $\bm U_1$ and $\bm V_1$ are  unitary and they have dimensions ${4\times 4}$ e $\frac{\ell}{4}\times\frac{\ell}{4}$ respectively. The diagonal entries $s_{i}= S_{ii}$ of $\bm S_1$ are known as the singular values of $\bm X_1$ and it have dimension $4\times\frac{\ell}{4}$ . The singular values are sorted in descending, and they are putting in the diagonal principal of the matrix, other entries must be zeros. 
...
\item 
Transform the lines of $\widehat{\bm X}_1=\bm U_1^T\bm X_1=\bm S_1 \bm V_1^T$ into new matrices with dimensions $\frac{m}{2}\times\frac{n}{2}$: $\{\bm\Phi_1, \bm\Psi_{1\text{V}}, \bm\Psi_{1\text{H}}, \bm\Psi_{1\text{D}}\}$. 
%\item Repeat the procedure on $\Phi_r$ $R$ times. Where $r=1,\dots,\text{R}$ and, R is lowest resolution index. 
\item Repeat the procedure (1) on $\bm\Phi_r$ by $r=2$ until lowest resolution level $R$. 
\item The MR-SVD method for each channel is represented as
\begin{equation}\nonumber
...
of $\bm U_r^c$ ($\bm U_\text{r}^f$), for each $r$. The supra-index $f$ shows the fusion among the channels. 
...
Fig.~(\ref{flevoland_radial_4look}) shows the ROI, with the radial lines where edges are detected. In Fig.~(\ref{gt_flevoland}) is showed a ground truth image.  
...
%\end{figure}
   \begin{figure}[hbt]
   \centering
     \subfloat[Flevoland image in Pauli decomposition, in Region of Interest (ROI) and rays. \label{flevoland_radial_4look}]{%
%       \includegraphics[viewport= 0 50 500 550, clip=true, width=0.23\textwidth]{flevoland_radial_4_look_black_crop}}      
     ...
     \subfloat[Ground truth - GT\label{gt_flevoland}]{%
       \includegraphics[width=0.216\textwidth]{gt_flevoland_crop}
     }
    \caption{ROI and GT AIRSAR PolSAR Flevoland image}
    \label{roi_gt}
\end{figure}
...
The figure~\ref{probability_edge_detc} shows the error that is measured by $100$ radial lines and finding $\widehat\jmath$. The ground truth pixel is considered as the real edge in each radial line, so the error for each radial line is the euclidean distance minimum of among the ground truth pixel and the several pixels detected in the fusion methods.

we use relative frequencies to estimate the probability of having an error smaller than a number of pixels. 
Denoting $H(k)$ the number of replications for which the error is less than $k$ pixels, an estimate of this probability is $f(k)=\frac{H(k)}{n_r}$, where $n_r$ is the radial number. 
In the tests performed to calculate , $k$ varies between $1$ and $10$. 
The algorithm is described in Ref.~\cite{fbgm}.
Fig.~\ref{probability_edge_detc} shows these probabilities as computed to the fusion methods. 

\begin{figure}[hbt]
\centering
	\includegraphics[width=0.60\linewidth]{metricas_6_fusao_flevoland}
...
The best result was produced by the Multi-Resolution Stationary Wavelet Transform (MR-SVD) with a moderate cost of the processing time.
