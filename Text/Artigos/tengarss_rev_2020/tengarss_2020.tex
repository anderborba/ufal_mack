...
$\text{I}_F(x,y)=(n_c)^{-1}\sum_{c=1}^{n_c} \widehat\jmath_c(x,y)$.
...
The filters DWT are applied separately in vertical and horizontal directions and down-sampled by a factor of two in image $\widehat\jmath_c$. In this way, the image is filtered by the low pass filter L in the vertical direction and the high pass filter H in the horizontal direction and then down-sampled by a factor of two to create the coefficients matrices $\widehat\jmath_{c\text{L}}$ and $\widehat\jmath_{c\text{H}}$. After this, the decompositions are submitted to the low pass and high pass filters again  in both direction and newly down-sampled by a factor of two to create sub-images named $\widehat\jmath_{c\text{LL}}$, $\widehat\jmath_{c\text{LH}}$, $\widehat\jmath_{c\text{HL}}$, and $\widehat\jmath_{c\text{HH}}$. The successive levels of decomposition can be achieved by repeating this procedure.
