...
\item Find the SVD decomposition of $\text{X}_1$, and calculate $\text{T}_1=\text{X}_1 \text{X}_1^\text{T}=\text{U}_1 \text{S}_1^2 \text{U}_1^\text{T}$, where $s(1)\geq s(2) \geq s(3) \geq s(4)$ compose the singular value matrix $\text{S}_1$, 
%%% ACF Não definiu nem usou s(1), s(2) etc
%%% AAB Os s(1), s(2), s(3) e s(4) são usados para gerar a matriz $S$
 and $\text U_1$ is unitary.
\item 
...
\item Repeat the procedure on $\Phi_r$. Where $r=1,\dots,\text{R}$ and, R is lowest resolution index. 
\item The MR-SVD method is represented as
\begin{equation}\label{msvd_iter}
\widehat{\text{X}}\leftarrow \left\{\Phi_\text{R},\{\Psi_r^\text{V},\Psi_r^\text{H},\Psi_r^\text{D} \}_{r=1}^\text{R},\{\text{U}_r\}_{r=1}^\text{R} \right\}.
\end{equation}
...
\item Compute $\overline{\Phi}_\text{R}$ averaged $\Phi_R$, and $\overline{\text{U}}_r$ averaged $\text{U}_r$ decompositions.
...
\item The fusion $\text{I}_\text{F}(x,y)$ is the SVD transformation to $\overline{\Phi}_R$, $\overline{\text{U}}_r$, $\overline{\Psi}_r^\text{V}$, $\overline{\Psi}_r^\text{H}$ and $\overline{\Psi}_r^\text{D}$,  for each level $r=1,\dots,\text{R}$. 
\end{enumerate}
