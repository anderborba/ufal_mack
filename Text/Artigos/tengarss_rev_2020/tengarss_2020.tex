...
\item Compute the matrices of eigenvalues ($\Lambda$) and eigenvectors (V) of the covariance matrix, sorting the eigenvalues and your respective eigenvectors in descending order. %The matrices generated by the eigenvalues, on the principal diagonal, and the eigenvectors placed in columns;
%%% ACF O que é V_c?
\item Compute the components $\text{P}_c=\frac{\text{V}_c}{\sum_{c=1}^{nc} \text{V}_c}$, where $\text{V}_c$ is an column de V. %with $i=1,\dots,nc$;
