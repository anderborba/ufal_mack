...
Os radares de abertura sintética (SAR) têm grande importância em inúmeras aplicações, contemplando diversas áreas do conhecimento, as quais, serão citadas ao longo deste texto. Neste trabalho estudaremos as imagens polarimétricas SAR (PolSAR) estas requerem uma base de dados com múltiplos valores complexos que exigem modelos e algoritmos adequados.

No estudo de imagens PolSAR podemos citar diferentes técnicas nas mais diversas áreas de pesquisa, por exemplo, na referência (\cite{slf_2008}) é usado modelagem eletromagnética para propor uma abordagem de detecção de borda nas simulações de imagens PolSAR. Nas referências (\cite{tlb}), (\cite{obw}), (\cite{flmc}) e (\cite{fyf}) podemos ver técnicas de detecção de bordas baseadas em métodos que estimam o gradiente, que funcionam usando uma janela deslizante, destacando através do cálculo do gradiente as bordas das imagens. Em (\cite{obw}) foi proposto o método de máxima verossimilhança para detectar a presença de bordas, em uma janela pré-definida de pixeis com a tarefa de encontrar bordas acuradas. O estudo de detecção de bordas, usando propriedades físicas das áreas estudadas, pode ser encontrado na referência (\cite{bf}), as quais utilizam cadeias de Markov. Em (\cite{gfn}) é descrito a comparação entre vários detectores de bordas que seguem a ideia deste trabalho. Técnicas baseadas nas modelagens estatísticas têm sido usadas na detecção de bordas em imagens SAR, podemos citar as referências (\cite{gmbf}), (\cite{fbgm}), (\cite{horrit}) e (\cite{gfn}). Atualmente as pesquisas em \textit{Deep Learning} são muito importantes na área de sensoriamento remoto, aplicações são encontradas nas referências (\cite{bac}), (\cite{ztmxzxf}), (\cite{tabmm}) e (\cite{xstz}). As técnicas de \textit{Deep Learning} são usadas para segmentar ou classificar imagens, podendo auxiliar no processo de detecção de bordas. 

A área de fusão de imagens também é explorada neste trabalho. Um recente e interessante artigo (\cite{sglmla}) usa ideias do método randon forest aplicado em fusão de imagens PolSAR, adicionalmente, o artigo (\cite{sg}) mostra outras técnicas de fusão de informação.  
