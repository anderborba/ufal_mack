...
\subsection{Estudo do artigo  \cite{gmbf2006}}
...
Para dados de amplitude o ruído {\it speckle} $Y$ é modelado como uma distribuíção randômica gamma $\Gamma^{-\frac{1}{2}}(n,n)$. Um modelo geral para o retroespalhamento $X$ considerado é a lei gaussiana inversa generalizada denotada por $N^{-\frac{1}{2}}(\alpha,\lambda,\gamma)$, descrita no artigo \cite{frery_muller_1997}.
...
As ideia apresentadas nesta seção são similares com as ideias do artigo \cite{nasc_frery_2014} e descritos na seção anterior, a mudança está na troca da função de densidade de probabilidades que será usada, nesta seção usaremos a função (\ref{eqn126}).
