...
Os radares de abertura sintética (SAR) têm grande importância em inúmeras aplicações, contemplando diversas áreas do conhecimento, as quais, serão citadas ao longo deste texto. 
%%% ACF Evite frases que não dizem nada, como a anterior
Neste trabalho estudaremos as imagens polarimétricas SAR (PolSAR) estas requerem uma base de dados com múltiplos valores complexos que exigem modelos e algoritmos adequados.
%%% ACF Precisa dizer por extenso, em itálico, o significado das siglas SAR e PolSAR

No estudo de imagens PolSAR podemos citar diferentes técnicas nas mais diversas áreas de pesquisa, 
%%% ACF O anterior não diz nada
por exemplo, no trabalho de \citet{slf_2008}
%%% ACF Use \citep e \citet
é usado modelagem eletromagnética para propor uma abordagem de detecção de borda nas simulações de imagens PolSAR. Nas referências (\cite{tlb}), (\cite{obw}), (\cite{flmc}) e (\cite{fyf}) 
%%% ACF Coloque todas as referências dentro da mesma chamada de citação
podemos ver técnicas de detecção de bordas baseadas em métodos que estimam o gradiente, que funcionam usando uma janela deslizante, destacando através do cálculo do gradiente as bordas das imagens. Em (\cite{obw}) foi proposto o método de máxima verossimilhança para detectar a presença de bordas, em uma janela pré-definida de pixeis com a tarefa de encontrar bordas acuradas. O estudo de detecção de bordas, usando propriedades físicas das áreas estudadas, pode ser encontrado na referência (\cite{bf}), as quais utilizam cadeias de Markov. Em (\cite{gfn}) é descrito a comparação entre vários detectores de bordas que seguem a ideia deste trabalho. Técnicas baseadas nas modelagens estatísticas têm sido usadas na detecção de bordas em imagens SAR, podemos citar as referências (\cite{gmbf}), (\cite{fbgm}), (\cite{horrit}) e (\cite{gfn}). 
...
Atualmente as pesquisas em \textit{Deep Learning} são muito importantes na área de sensoriamento remoto, aplicações são encontradas nas referências (\cite{bac}), (\cite{ztmxzxf}), (\cite{tabmm}) e (\cite{xstz}). As técnicas de \textit{Deep Learning} são usadas para segmentar ou classificar imagens, podendo auxiliar no processo de detecção de bordas. 

A área de fusão de imagens também é explorada neste trabalho. Um recente e interessante artigo (\cite{sglmla}) usa ideias do método \textit{random forest} aplicado em fusão de imagens PolSAR, adicionalmente, o artigo (\cite{sg}) mostra outras técnicas de fusão de informação.  

O presente trabalho seguirá a abordagem de modelagem estatística, principalmente as técnicas descritas em (\cite{fbgm}) e (\cite{nhfc}) usando a distribuição Wishart. Para realizar a fusão de informações temos como base as referências (\cite{mit}), (\cite{sglmla}) e (\cite{sg}). 
...
Os itens (i) até (iv) tratam da detecção de borda que foram propostos nas seguintes referências bibliográficas (\cite{gmbf}), (\cite{gmbf_sc}) e (\cite{fbgm}), esses artigos usam a distribuição $G^{0}$ proposta por (\cite{fmcs}). Neste trabalho usaremos a distribuição Wishart como na referência (\cite{nhfc}). 

No item (v) a principal referência usada para fusão de evidências foi (\cite{mit}), no item (vi) para o método dos quadrados mínimos foi usado a referência (\cite{demmel}).
...
Os radares podem produz imagens bidimensionais (2D) com resoluções distintas, dependendo do tipo de radar e da tecnologia empregada. As dimensões das imagens dependem respectivamente da resoluções nas direções de azimute e distância ({\it range}). A direção de azimute é a mesma direção da rota do radar sendo perpendicular à direção da distância. A resolução na direção de azimute depende do comprimento $l$ da abertura antena de radar.  
...
%%% ACF Use \centering ao invés de \begin{center}, use o pacote booktabs, e coloque as legendas acima da tabela. Localize todos os elementos flutuantes com [hbt]
%%% ACF Use o pacote siunitx para escrever unidades
\end{center}
	\begin{table}[h!]
		\centering
\begin{tabular}{ |c|c| } 
...
\hline
	Características&  \\
\hline
	Frequência           & $1.275$ Ghz  \\ 
	Altitude             & $780$ Km   \\
	Peso                 & $2300$ kg   \\
	Ângulo de inclinação & $\sim 23^{o}$   \\
	Distância {\it range}& $100$ Km  \\
	Largura de banda     & $19$ Mhz   \\
	banda - L            & $23.5$ Cm de comprimento de onda\\
	Polarização          & HH, onda emitida e recebida na direção horizontal \\
	Resolução            & $25 \times 25$  \\
	\hline
\end{tabular}
	\caption{Características do satélite SEASAT(SAR).}
	\label{cap_acf_tab01}
\end{table}
...
%%% ACF Use tabelas na vertical com o pacote rotating
\begin{center}
	\begin{table}[h!]
	\tiny
		\centering
...
\hline
Satélite      & 	SEASAT  &AIRSAR &SIR-C& Almaz&ERS-2& JERS-1& RADSAT-1&RADSAT-2 \\
\hline\hline
Nacionalidade       &EUA    &EUA&Alamanha-Itália&Rússia&Europa&Japão&Canada&Canada  \\\hline 
Lançamento          &1978   &1988        &1990  &1992  &1995 &1998  &1995  & 2003\\\hline
Compr. de onda & 23.5(L)&67(P)/23.5(L)/5.7(C)&23.5(L)/5.7(C)/3.2(X)&10(S)&5.7(C)&23.5(L)&5.6(C)&5.6(C)\\\hline
Polarização         &$HH$&$HH/HV/VV$&$HH/HV/VV$&$HH$&$VV$&$HH$&$HH$&$HH/HV/VV$\\\hline
âng. de incidência&23&20-60&15-55&30-60&23&35&20-59&20-60\\\hline
Distância(Km)           &100&10-17&15-90&350&100&75&50-500&10-500\\\hline
Resolução(m)         &25&2-8&10-60&10-30&30&18&10-100&3-100\\\hline
\end{tabular}
	\caption{Características dos satélites SAR e PolSAR.}
	\label{cap_acf_tab02}
\end{table}
\end{center}
...
\begin{tabular}{ |c|c|c| } 
\hline
	banda & Frequência$f(Ghz)$ & Freq$\times$Comprimento de onda $\lambda(cm)$. \\
\hline
	...
	$W$&$(56.0- >56.0)$  & $100.0\times 0.3$ \\ 
\hline
\end{tabular}
	\caption{Espectro magnético para a faixa de micro-ondas.}
	\label{cap_acf_tab03}
\end{table}
\end{center}
%
...
\hline
\end{tabular}
	\caption{Parâmetros estatísticos da matriz de variância.}
	\label{cap_acf_tab04}
\end{table}
\end{center}
...
\hline
	 $HH$ &$HV$&$VV$ &$HHHV(\mathbf{Re})$ &$HHHV(\mathbf{Im})$&$HHVV(\mathbf{Re})$&$hhvv(\mathbf{Im})$ &$HVVV(\mathbf{Re})$&  $HVVV(\mathbf{Im})$ \\
\hline
	$\overline{\sigma_{hh}}$&$\overline{\sigma_{hv}}$&$\overline{\sigma_{vv}}$&$\overline{\sigma_{hhhv}}$&$\overline{\bar{\sigma}_{hhhv}}$&$\overline{\sigma_{hhvv}}$&$\overline{\bar{\sigma}_{hhvv}}$&$\overline{\sigma_{hvvv}}$&$\overline{\bar{\sigma}_{hvvv}}$\\
\hline
\end{tabular}
	\caption{Ordem de armazenamento para os canais com um pixel fixo.}
	\label{cap_acf_tab05}
\end{table}
\end{center}

Em imagens sintéticas podemos encontrar a aproximação $\overline{\sigma_{ij}}$, com $i$, $j$ $\in \{HH,HV,VV\}$ usando o método de monte carlo com o auxílio da matriz de covariância $\Sigma$. Em imagens reais teremos que analisar $\overline{\sigma_{ij}}$, dependendo da técnica utilizada para definição dos parâmetros estatísticos nas regiões de interesse, podemos citar os artigos (\cite{gmbf}) e (\cite{nhfc}) para analisar os diferentes métodos, reforçando que a descrição do método proposto no segundo artigo será aplicado ao longo deste trabalho.

Um exemplo clássico de imagem PolSAR é da baía de São Francisco (US), com suas respectivas polarizações em tons de cinza, mostradas na figura (\ref{cap_acf_sf_hh_hv_vv}), 
\begin{figure}[hbt]
\includegraphics[width=\linewidth]{sf_hh_hv_vv.jpg}
\caption{Imagem PolSAR com polarizações $HH$, $HV$ e $VV$.}
\label{cap_acf_sf_hh_hv_vv}
\end{figure}
...
\end{equation}
%%% ACF Evite o uso excessivo de parênteses
...
%%% ACF Uma estimativa não é uma aproximação
...
De acordo com a função densidade de probabilidade (\ref{cap_acf_23}) e usando o canal $(hh)$ em ambas matrizes de covariância com $L=4$, podemos gerar as figurar (\ref{cap_acf_fig02}) e (\ref{cap_acf_fig03}), com a representação gráfica da função de densidade para os respectivos elementos $u$ e $f$ da matriz de covariância, neste caso, $\sigma_{hh}=962892$ e $\sigma_{hh}= 360932$. 
...
\endminipage\hfill
\end{figure}
%%% ACF Faltou dizer que são os modelos para as regiões à direita e à esquerda
...
\end{figure}
	...
	Podemos notar que as funções não são deriváveis em muitos pontos, então os métodos de otimização que necessitam do cálculo da derivada, terão funcionamento comprometido, resolvemos esse problema usando o método Simulated Anneling Generalizado (GenSA) que podemos encontrar na referência \cite{xgsh}.
	
	O método GenSA mostrou-se competitivo com os métodos empregados no artigo \cite{nhfc}, para testar a acurácia do GenSA e fazer a comparação com os métodos aplicados no artigo, vamos usar a métrica encontrada no artigo \cite{nhfc} e proposta na referência \cite{fbgm}.
...
\endminipage\hfill
\end{figure}
%%% ACF Nunca use JPG para gerar gráficos; use PDF
...
\section{Fusão de evidências de bordas}
	...
	Nesta seção será descrito como realizamos a fusão de evidências de bordas, a referência adotada foi \cite{mit} que mostra várias técnicas de fusão de imagens. 
