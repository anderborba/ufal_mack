...
%%% AAB Realizado
Os conceitos apresentado nesta seção são baseado nos artigos \citet{ade} e \citet{fnc2011}, assim como no livro \citet{lp}.
...
\end{equation}
\index{Span}
onde o operador $\traco(\cdot)$ é o traço de uma matriz, e o superíndice $H$ denota o a matriz transposta conjugada.
...
Vamos escolher duas bases, a primeira chamada de  base de Pauli:
...
Em meio recíprocos podemos definir a igualdade dos termos de polarização cruzada $S_\text{hv}=S_\text{vh}$; ver \citet{lp}. 
...
\label{vetor_3d}
\end{equation}
a potência total espalhada no caso de um sistema de radar polarimétrico em meio recíproco é, então:
\begin{equation}
...
Por último, o espalhamento volume-superfície consiste em a onda atingir outro de meio de propagação, por exemplo o solo de uma floresta.
...
As ondas recebidas de cada elemento espalhador, embora coerentes em frequência, não são coerentes em fase. 
O sinal é forte se as ondas são construtivas, ou seja em fase, e fraco se a ondas não estão em fase, ou seja se estão espalhadas de forma destrutiva.
...
Se o sinal é resultante da interação de $M$ elementos espalhadores, então $x+\hat{\jmath}y$ é soma destes elementos, e $r\exp(\hat{\jmath}\theta)$ é a decomposição de Euler para o número complexo $x+\hat{\jmath}y$.
...
Em ambos casos, o coeficiente de variação é constante, portanto não é possível melhorar a relação sinal-ruído aumentando o valor médio da imagem.
...
Sob a hipótese da eq.~\eqref{eq:DensidadeExponencial} decorre que a função densidade de probabilidade para os canais de intensidades com múltiplas visadas é dada por
\begin{equation}
f_I(z_I;L,\sigma^2)=\frac{L^L z_I^{L-1}}{(L-1)!\sigma^{2L}}\exp\left(-L\frac{z_I}{\sigma^2}\right), z_I\geq 0.
\label{pdf_inten_multilook}
\end{equation}
Assim, a intensidade multilook obedece uma distribuição gama.
...
O coeficiente de variação é calculado pela razão entre o desvio padrão e a média, resultando para o caso de múltiplas visadas em $\text{CV}_L(z_I)=\frac{1}{\sqrt{L}}$. Podemos observar que o coeficiente de variação é reduzido por $\sqrt{L}$ com relação ao processo de visada simples.
...
Para esse fim, matrizes positivas definidas hermitianas estimadas são obtidas computando a média de $L$ visadas independentes de uma mesma cena, 
...
A figura~\ref{fig:wishart_multi_visadas} mostra a distribuição wishart com os parâmetros fixos $\sigma=0.01$ e o número de visadas $L=2,3,4,8$.  
\begin{figure}[hbt]
\centering
...
Considerando a função densidade de probabilidade da distribuição complexa Wishart dada em~\eqref{eq:dist_whishart}, vamos usar o logaritmo natural e suas propriedades para reescrever a função na forma adequada para aplicações ao longo do texto:
\begin{equation}\nonumber
\begin{array}{lcl}
...
\begin{itemize}
	...
	\item Usadar o método MLE, juntamente com o método \textit{simulated annealing};
...
A figura~\ref{fig:cap_det_bor_radiais} mostra a região de interesse ROI-$\mathbf{R}$ com centroide $\mathbf{C}$, de onde traçamos raios até um ponto de controle $\mathbf{P}_i$, 
%%% ACF \mathbf S foi usado na parte de modelagem PolSAR. Troque a notação.
com $i=1,2,\dots, \mathbf{S}$, estes pontos de controle estão na região externa da ROI-$\mathbf{R}$. 
Teremos $\mathbf{S}$ raios resultantes representados por $\bm{s}^i	=\overline{\mathbf{CP}}_i$ com ângulos $\epsilon_{i}=\angle(\bm s^i,\bm s^{i+1})$. 
...
O ponto que maximiza da função~\eqref{eq:LogLikelihoodGamma_red} é encontrado com o método BFGS, implementado no pacote \texttt{maxLik} \citep{ht}. 
