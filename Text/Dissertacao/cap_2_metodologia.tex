...
\end{equation}
para mais detalhes podemos consultar \citet{ljdwo}.

\section{Metodologia}

\section{Detecção das evidências de bordas}
\section{Modelos estatísticos para os dados SAR/PolSAR}
\subsection{Estimativa de parâmetros usando o método da máxima verossimilhança}
\subsection{Função densidade de probabilidade univariada}
\subsubsection{Função densidade de probabilidade $\Gamma$ para os canais de intensidades}
...
Dado a amostra $\bm z = (z_1,\dots,z_n)$ extraída dos canais de intensidades hh, hv, e vv, deduzimos a função log-verossimilhança,  
\begin{equation}\nonumber
\begin{split}
 ...
 \end{split}
 \end{equation}
com o uso da função logarítimica~\eqref{func_log_univ_gaussiana}, teremos
\begin{equation}\nonumber
\begin{split}
    \ell(\bm z;\mu, L)&=\sum_{k=1}^{n}\ln f_Z(z_k;\mu,L)\\
...
\begin{equation}\nonumber
 ...
 \end{equation}
e a sua forma reduzida chamada de função log-verossimilhança reduzida, 
\begin{equation}
\ell(\bm z;\mu, L) = 
...
Extraindo de cada canal de intensidade da imagem PolSAR uma faixa de dados $\bm z = (z_1,z_2,\dots,z_n)$ de forma que seja particionada em duas amostras disjuntas na posição $j$:  
$$
...
Destacamos que podemos usar o método de otimização BFGS para obter uma estimativa dos parâmetros ($\mu, L$) simultaneamente, isto é, otimizamos funções com duas variáveis, ou, alternativamente, podemos estimar, ou fixar, o número de visadas $L$ a priori e otimizar uma função de uma variável em $\mu$. Derivando a equação~\eqref{eq:LogLikelihoodGamma_red} com relação a variável $\mu$ e igualando a zero, encontramos  
...
\label{eq:est_media_mu}
\end{equation}
a qual podemos usar como estimativa inicial para obter os parâmetros $(\widehat{\mu}, \widehat{L})$ no processo de otimização com duas variáveis, ou como aproximação para o parâmetro $\mu$ quando L fixo ou estimado a priori. 
\begin{equation}
...
Quando optamos por usar o L estimado a priori e a média~\eqref{eq:est_media_mu} como estimativa de $\mu$ usamos o seguinte procedimento. Considerando a faixa de dados extraída da imagem e a partição da mesma em
$\bm Z_\text{I} \sim \Gamma(\mu_\text{I},L)$, e
$\bm Z_\text{E} \sim \Gamma(\mu_\text{E},L)$.
Podemos estimar $\mu_\text{I}$ e $\mu_\text{E}$ com as amostras $\bm z_\text{I}$ e $\bm z_\text{E}$ calculando a média~\eqref{eq:est_media_mu} para cada amostra. Desta forma teremos os parâmetros $(\widehat{\mu}_\text{I}, L)$ e $(\widehat{\mu}_\text{E}, L)$ conhecidos resultando na função log-verossimilhança no ponto $j$, 
\begin{equation}\label{eq:TotalLogLikelihood_L_fixo}
\begin{split}
\ell(j&;\widehat{\mu}_I,\widehat{\mu}_E, L)= 
 -j\ln \hat{\mu}_\text{I} 
 - \frac{L}{\widehat{\mu}_\text{I}}\sum_{k=1}^{j} z_k 
 -(n-j)\ln \widehat{\mu}_\text{E}
...
sensor para aquisição de imagem. 
%
\subsubsection{Função densidade de probabilidade produto de magnitudes das intensidades}
%
A  função densidade de probabilidade univariada produto de magnitudes das intensidades é definida por
...
Aplicando  o logaritmo natural na equação~\eqref{eq:pdf_mag_prod}, teremos
\begin{equation}\nonumber
\begin{split}
	...
	&+\ln I_0\left(\frac{2|\rho|Lz}{1-|\rho|^2}\right)+ \ln K_{L-1}\left(\frac{2Lz}{1-|\rho|^2}\right),
		\end{split}
\end{equation}
então definimos a função logarítmica
...
A função log-verossimilhança pode ser deduzida sendo dado a amostra $\bm z = (z_1,\dots,z_n)$ obtidas das imagens SAR/PolSAR 
\begin{equation}\nonumber
\begin{split}
 ...
 \end{split}
 \end{equation}
usando a função logarítmica~\eqref{eq:log_vero_mag_produto}, teremos
\begin{equation}\nonumber
\begin{split}
    \ell(\bm z;\rho, L)&=\sum_{k=1}^{n}\ln f(z_k;\rho,L)\\
...
Definimos a função log-verossimilhança para a PDF~\eqref{eq:pdf_mag_prod}
\begin{equation}\nonumber
\begin{split}
...
Alternativamente podemos resolver o problema da seguinte forma, considerando o número de visadas $L$ conhecido ou estimado a priori podemos reescrever a equação \eqref{eq:eq_log_vero_mag_prod_red} por
...
A função log-verossimilhança no ponto $j$ com $L_\text{I}=L_\text{E}=L$ definidas a priori é
...
\end{split}
\end{equation}
e aplicando o método GenSA para encontrar
$$
\widehat{\jmath}= \arg\max\limits_{j\in [\min_s,N-\min_s]}\ell(j;\widehat{\rho}_I,\widehat{\rho}_E),
$$ 
onde $\min_s$ é o tamanho da folga em cada extremidade da amostra definida empiricamente.
%
\subsubsection{Função densidade de probabilidade razão de intensidades}
A razão de intensidades ou amplitudes entre $\mathbf{S}_i$ e $\mathbf{S}_j$ são importantes no estudo de radares polarimétricos. 

Seja a razão de intensidade normalizada,
...
\end{equation}
onde $\tau=\frac{\Sigma_{11}}{\Sigma_{22}}$, e a função densidade de probabilidade razão de intensidades,
...
\end{equation}
onde, $\rho>0$ e $L>0$. Definindo 
\begin{equation}\label{eq:razao_intensidades_z}
...
\begin{equation}\label{eq:pdf_razao_intensidades_tau_z}
	f(z;\rho,L,\tau)=\frac{\tau^L\Gamma(2L)(1-|\rho|^2)^{L}(\tau+z)z^{L-1}}{\Gamma(L)\Gamma(L)\left[(\tau+z)^2-4\tau|\rho|^2z \right]^{\frac{2L+1}{2}}}.\\
\end{equation}

Aplicando o logaritmo natural na equação~\eqref{eq:pdf_razao_intensidades_tau_z} e realizando manipulações algébricas teremos
\begin{equation}\nonumber
\begin{split}
	\ln f(z;\rho,L,\tau)&=\ln\left(\frac{\tau^L\Gamma(2L)(1-|\rho|^2)^{L}(\tau+z)z^{L-1}}{\Gamma(L)\Gamma(L)\left[(\tau+z)^2-4\tau|\rho|^2z \right]^{\frac{2L+1}{2}}}\right),\\
	                &=\ln\left(\tau^L\Gamma(2L)(1-|\rho|^2)^{L}(\tau+z)z^{L-1}\right)\\
	                &-\ln\left(\Gamma(L)\Gamma(L)\left[(\tau+z)^2-4\tau|\rho|^2z \right]^{\frac{2L+1}{2}}\right),\\
	                &=\ln\tau^L + \ln\Gamma(2L) +\ln(1-|\rho|^2)^{L}+\ln(\tau+z)+\ln z^{L-1}\\
	                &-\left(\ln\Gamma(L)+\ln\Gamma(L)+\ln\left[(\tau+z)^2-4\tau|\rho|^2z \right]^{\frac{2L+1}{2}}\right),\\
	                &=L\ln\tau + \ln\Gamma(2L) +L\ln(1-|\rho|^2)+\ln(\tau+z)+(L-1)\ln z\\
	                &-2\ln\Gamma(L)-\frac{2L+1}{2}\ln\left[(\tau+z)^2-4\tau|\rho|^2z \right].\\
\end{split}
\end{equation}

Definimos a função logarítmica para a PDF razão de intensidades,
\begin{equation}\label{eq_log_vero_razao_intensidade_tau_z}
\begin{split}	
	\ln f(z;\rho,L,\tau)&=L\ln\tau + \ln\Gamma(2L) +L\ln(1-|\rho|^2)+\ln(\tau+z)+(L-1)\ln z\\
	                      &-2\ln\Gamma(L)-\frac{2L+1}{2}\ln\left[(\tau+z)^2-4\tau|\rho|^2z \right].\\
\end{split}
\end{equation}

Dado a amostra $\bm z = (z_1,\dots,z_n)$ obtida das imagens SAR/PolSAR, deduzimos a função log-verossimilhança 
\begin{equation}\nonumber
\begin{split}
  \ell(\bm z;\rho, L, \tau)=\ln\prod_{k=1}^{n}f(z_k;\rho,L, \tau)\\
  \ell(\bm z;\rho, L, \tau)=\sum_{k=1}^{n}\ln f(z_k;\rho,L, \tau),
 \end{split}
 \end{equation}
usando a função~\eqref{eq_log_vero_razao_intensidade_tau_z}, teremos,
\begin{equation}\nonumber
\begin{split}
    \ell(\bm z;\rho, L, \tau)&=\sum_{k=1}^{n}\ln f(z_k;\rho, L, \tau)\\
                         &=\sum_{k=1}^{n}\left[L\ln\tau + \ln\Gamma(2L) +L\ln(1-|\rho|^2)+\ln(\tau+z_k)+(L-1)\ln z_k\right.\\
	                     &-\left.2\ln\Gamma(L)-\frac{2L+1}{2}\ln\left[(\tau+z_k)^2-4\tau|\rho|^2z_k \right]\right],\\
 \end{split}
 \end{equation}
 \begin{equation}\nonumber
\begin{split} 
    \ell(\bm z;\rho, L, \tau)&=L\ln\tau\sum_{k=1}^{n}1+\ln\Gamma(2L)\sum_{k=1}^{n} 1+L\ln(1-|\rho|^2)\sum_{k=1}^{n} 1+\sum_{k=1}^{n}\ln(\tau+z_k)+(L-1)\sum_{k=1}^{n}\ln z_k\\
                         &-2\ln\Gamma(L)\sum_{k=1}^{n} 1-\frac{2L+1}{2}\sum_{k=1}^{n}\ln\left[(\tau+z_k)^2-4\tau|\rho|^2z_k\right]\\
                         &=n\left(L\ln\tau+\ln\Gamma(2L)+L\ln(1-|\rho|^2)-2\ln\Gamma(L)\right)+\sum_{k=1}^{n}\ln(\tau+z_k)\\
                         &+L\sum_{k=1}^{n}\ln z_k-\sum_{k=1}^{n}\ln z_k-\frac{2L+1}{2}\sum_{k=1}^{n} \ln\left[(\tau+ z_k)^2-4\tau|\rho|^2z_k\right].\\
\end{split}
\end{equation}
 
Definimos a equação log-verossimilhança para a PDF univariada~(\ref{eq_log_vero_razao_intensidade_tau_z}).
\begin{equation}\nonumber
\begin{split}
    \ell(\bm z;\rho, L, \tau)&=n\left(L\ln\tau + \ln\Gamma(2L)+L\ln(1-|\rho|^2)-2\ln\Gamma(L)\right)+\sum_{k=1}^{n}\ln(\tau+z_k)\\
                         &+L\sum_{k=1}^{n}\ln z_k-\sum_{k=1}^{n}\ln z_k-\frac{2L+1}{2}\sum_{k=1}^{n} \ln\left[(\tau+z_k)^2-4\tau|\rho|^2z_k\right]\\
\end{split}
 \end{equation}
e a forma reduzida,
\begin{equation}\label{eq_log_vero_razao_intensidade_red_tau_z}
\begin{split}
...
Vamos obter $(\widehat \rho, \widehat L, \widehat \tau)$, o estimador de máxima verossimilhança (MLE) de $(\rho, L, \tau)$ baseado na amostra $\bm z$ maximizando~\eqref{eq_log_vero_razao_intensidade_red_tau_z} com o método BFGS implementado no pacote \texttt{maxLik}~\citep{ht}. Vamos preferir otimização resolvendo $\nabla\ell=\bm 0$ com intuito de melhorar a estabilidade numérica.

A função log-verossimilhança reduzida é aplicada na faixa de dados $\bm z = (z_1,z_2,\dots,z_n)$ particionada em duas amostras disjuntas na posição $j$,  denotadas respectivamente como $\bm z_\text{I}$ e $\bm z_\text{E}$,  
$$
\bm z = (\underbrace{z_1,z_2,\dots,z_j}_{\bm z_\text{I}}, 
\underbrace{z_{j+1}, z_{j+2},\dots,z_n}_{\bm z_\text{E}}).
$$

Para cada amostra estimamos os parâmetros $(\rho_\text{I},L_\text{I}, \tau_\text{I})$ e $(\rho_\text{E},L_\text{E}, \tau_\text{E})$ maximizando~\eqref{eq_log_vero_razao_intensidade_red_tau_z}, obtendo $(\widehat{\rho}_\text{I}, \widehat{L}_\text{I}, \widehat{\tau}_\text{I})$ e $(\widehat{\rho}_\text{E}, \widehat{L}_\text{E}, \widehat{\tau}_\text{E})$. Usando as estimativas para cada pixel podemos definir a função log-verossimilhança total no ponto $j$,
...
\ell(j;\widehat{\rho}_\text{I}, \widehat{L}_\text{I}, \widehat{\tau}_\text{I}, \widehat{\rho}_\text{E}, \widehat{L}_\text{E},\widehat{\tau}_\text{E})&=n\left(\widehat{L}_\text{I}\ln\widehat{\tau}_\text{I} +\ln\Gamma(2\widehat{L}_\text{I})+\widehat{L}_\text{I}\ln(1-|\widehat{\rho}_\text{I}|^2)-2\ln\Gamma(\widehat{L}_\text{I})\right)\\
                         &+\sum_{k=1}^{n}\ln(\widehat{\tau}_\text{I}+z_k)+\widehat{L}_\text{I}\sum_{k=1}^{n}\ln z_k-\frac{2\widehat{L}_\text{I}+1}{2}\sum_{k=1}^{n} \ln\left[(\widehat{\tau}_\text{I}+z_k)^2-4\widehat{\tau}_\text{I}|\widehat{\rho}_{I}|^2z_k\right]\\
                         ...
                         &+\sum_{k=1}^{n}\ln(\widehat{\tau}_\text{E}+z_k)+\widehat{L}_\text{E}\sum_{k=1}^{n}\ln z_k-\frac{2\widehat{L}_\text{E}+1}{2}\sum_{k=1}^{n} \ln\left[(\widehat{\tau}_\text{E}+z_k)^2-4\widehat{\tau}_\text{E}|\widehat{\rho}_\text{E}|^2z_k\right]
%\raisetag{2.2em}
\end{split}
\end{equation}
...
Alternativamente, sendo o número de visadas $L$ conhecido ou estimado a priori, podemos redefinir a função log-verossimilhança reduzida por 
\begin{equation}\label{eq:log_razao_int_z_L_fixo}
\begin{split}
    \ell(\bm z;\rho, \tau)&=n\left(L\ln\tau+L\ln(1-|\rho|^2)\right)\\
                         &+\sum_{k=1}^{n}\ln(\tau+z_k)
                         -\frac{2L+1}{2}\sum_{k=1}^{n} \ln\left[(\tau+z_k)^2-4\tau|\rho|^2z_k\right].\\
\end{split}
 \end{equation}    
 
Maximizando a função $\ell(\bm z;\rho, \tau)$ com o objetivo de estimar $(\rho_\text{I}, \tau_\text{I})$ e $(\rho_\text{E}, \tau_\text{E})$ para as amostras $\bm z_\text{I}$ e $\bm z_\text{E}$, obtemos $(\widehat{\rho}_\text{I},\widehat{\tau}_\text{I})$ e $(\widehat{\rho}_\text{E}, \widehat{\tau}_\text{E})$. O método de otimização BFGS implementado no pacote \texttt{maxLik}~\citep{ht}
...
\ell(j;\widehat{\rho}_\text{I}, \widehat{\tau}_\text{I}, \widehat{\rho}_\text{E},\widehat{\tau}_\text{E})&=
                         ...
                         &+\sum_{k=1}^{n}\ln(\widehat{\tau}_\text{I}+z_k)
                         -\frac{2L+1}{2}\sum_{k=1}^{n} \ln\left[(\widehat{\tau}_\text{I}+z_k)^2-4\widehat{\tau}_\text{I}|\widehat{\rho}_{I}|^2z_k\right]\\
                         &+n\left(L\ln\widehat{\tau}_\text{E}
                          +L\ln(1-|\widehat{\rho}_\text{E}|^2)\right)\\
                         &+\sum_{k=1}^{n}\ln(\widehat{\tau}_\text{E}+z_k)
                          -\frac{2L+1}{2}\sum_{k=1}^{n} \ln\left[(\widehat{\tau}_\text{E}+z_k)^2-4\widehat{\tau}_\text{E}|\widehat{\rho}_\text{E}|^2z_k\right].
%\raisetag{2.2em}
\end{split}
\end{equation}
...
$$ 
onde $\min_s$  é o tamanho da folga em cada extremidade da amostra definida empiricamente.
\subsubsection{Funçao distribuição de densidade para o span em meio recíproco}
...
\begin{equation}\nonumber
\begin{array}{ccl}
	\ln f_{S}(s;\mu,L)&=&\ln \left(\frac{L^{L}}{\Gamma(L)\mu^{L}} s^{L-1}\exp\left\{-\frac{L}{\mu}s\right\}\right), \\
	                                         &=&\ln\left(\frac{L}{\mu}\right)^L-\ln\Gamma(L)+ \ln s^{L-1} + \ln \exp\left\{-\frac{L}{\mu}s\right\}, \\
...
A função log-verossimilhança pode ser deduzida da seguinte maneira, dado a amostra $\bm s = (s_1,\dots,s_n)$ obtida das imagens SAR/PolSAR, 
\begin{equation}\nonumber
\begin{split}
...
O função é a log-verossimilhança reduzida é aplicada para as amostras internas e externas da faixa de dados denotadas respectivamento como $\bm s_\text{I}$ e $\bm s_\text{E}$. Cada faixa de dados $\bm s = (s_1,s_2,\dots,s_n)$ é particionada em duas amostras disjuntas na posição $j$:  
$$
...
onde $\min_s$ é o coeficiente de folga definido empiricamente.
 ...
 Alternativamente, podemos assumir o número de visadas L fixo, ou estimado a priori, e otimizar a função
\begin{equation}
\ell(\bm s;\mu) = 
...
Assim, encontramos estimativas~$\widehat{\mu}_\text{I}$~e~$\widehat{\mu}_\text{E}$~para os parâmetros~$\mu_\text{I}$~e~$\mu_\text{E}$~com as amostras~$\bm S_\text{I}$~e~$\bm S_\text{E}$ usando as médias para a mostra de acordo com a equação~\eqref{eq:est_media_mu}.

Usando as estimativa~$\widehat{\mu_\text{I}}$~e~$\widehat{\mu_\text{E}}$, a log-verossimilhança total no ponto $j$ é, então
...
\subsection{Função densidade de probabilidade bivariada}
\subsubsection{Função densidade de probabilidade produto de intensidades} 
Seja a função distribuição de probabilidade bivariada produto de intensidades, a qual pode ser encontrada em \citet{lee}
\begin{equation}\label{func_biv_produto_inten_z1_z2}
...
Aplicando o logaritmo natural em ambos os lados na equação \eqref{func_biv_produto_inten_z1_z2}
\begin{equation}\nonumber
\begin{split}
	\ln f(z_1,z_2;\rho, L, \Sigma_{11}, \Sigma_{22})&=\ln\left(\frac{L^{L+1}\left(z_1z_2\right)^{\frac{L-1}{2}}\exp\left(-\frac{L\left(\frac{z_1}{\Sigma_{11}}
	+\frac{z_2}{\Sigma_{22}}\right)}{1-|\rho|^2}\right)}{(\Sigma_{11}\Sigma_{22})^{\frac{L+1}{2}}\Gamma(L)(1-|\rho|^2)|\rho_c|^{L-1}}I_{L-1}\left(2L\sqrt{\frac{z_1z_2}{\Sigma_{11}\Sigma_{22}}}\frac{|\rho|}{1-|\rho|^2}\right)\right),\\
	\end{split}
\end{equation}
\begin{equation}\nonumber
\begin{split}
...
A função log-verossimilhança pode ser deduzida da seguinte maneira, sendo dado as amostras $\bm z_1 = (z^1_1,\dots,z^1_n)$ e  $\bm z_2 = (z^2_1,\dots,z^2_n)$ obtidas das imagens SAR/PolSAR, 
\begin{equation}\nonumber
\begin{split}
...
Definimos a equação log-verossimilhança para a PDF bivariada~(\ref{fun_log_biv_inten})
\begin{equation}\nonumber
\begin{split} 
...
Alternativamente, sendo o número de visadas $L$ conhecido ou estimado a priori, reescrevemos a função logarítmica~\eqref{eq:log_vero_biv_prod_red}
...
A log-verossimilhança no ponto $j$ usando os parâmetros estimados é
\begin{equation}\label{eq:TotalLogLikelihood_biv_prod}
\begin{split}
...
\end{split}
\end{equation}
onde podemos aplicar o método GenSA para encontrar
$$
