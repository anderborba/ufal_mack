...
Os radares que dependem diretamente do comprimento da sua antena são conhecidos com radares tipo RAR, os quais têm uma grande restrição de uso em sensoriamento remoto, pois as dimensões da antena inviabilizam o uso em satélites ou aviões, configurando-se em grande problema para essa área de pesquisa até os anos 50.  

Na década de 50, pesquisadores fizeram avanços significativos para resolver as limitações tecnológicas dos radares e desenvolveram uma técnica capaz de sintetizar o efeito de uma antena muito longa, em uma antena de tamanho viável, denominada SAR. 
...
\item Sintetiza longas aberturas de antenas; %%% ACF Me parece irrelevante, e confuso, dado o item anterior
\item Os radares produzem imagens dia e noite;
\item As condições atmosfériaas interfere muito pouco na captação de imagens.
\end{itemize}
...
A tabela~\ref{tab:micro_onda_espectro} mostra a definição das faixas de micro-ondas que os radares construídos. 
Denotamos a frequência $f$, e o comprimento de onda $\lambda$.
...
\begin{tabular}{@{}llc@{}} \toprule
	banda & Frequência [\si{\giga\hertz}] & $f\times\lambda$ [\si{\centi\meter}]. \\\midrule
...
A descoberta da tecnologia SAR impulsionou um grande desenvolvimento, levando à construção do SEASAT, o primeiro SAR comercial projetado. 
Este projeto foi muito bem sucedido e estabeleceu definitivamente os sistemas SAR e PolSAR como área de pesquisa e de aplicação. 
...
Outros projetos de sistema SAR e PolSAR foram lançados e podem ser vistos na tabela \eqref{tab:carac_radares}. Neste trabalho serão usadas imagens capturadas pelo radar AIRSAR, cuja características estão na Tabela~\ref{tab:carac_radares}.
...
O sistema PolSAR mono-estático gera informações no formato mostrado na Tabela~\ref{tab:sistema_polsar}.
...
Para cada pixel, na terceira dimensão da matriz, definidos como canais, são colocados os valores mostrados na Tabela~\ref{tab:sistema_polsar}.

Para cada ponto fixo da imagem, os canais estão dispostos conforme a Tabela~\ref{tab:canais},
...
Um exemplo de dados PolSAR é a imagem da baía de São Francisco (EUA), com suas respectivas polarizações em tons de cinza, mostradas na figura~\ref{fig:sf_hh_hv_vv}. 
Com essas imagens será realizar o processamento de dados com o objetivo de detectar bordas. 
...
Um exemplo disso é o sistema Lynx, projetado pelo laboratório nacional Sandia, o qual alcança resoluções de $10$ a $30$ centímetros quadrados.
%%% ACF Fornecer referência ou link

Outro exemplo, pode ser visto no trabalho de \cite{rijpbnm}, que mostra um sistema PolSAR que produzem imagens com resolução de 
%%% ACF Use siunitx
$25\times 25$ centímetros, 
conhecido como F-SAR, desenvolvido pelo centro aeroespacial da Alemanha (DLR). 
Esse sistema pode adquirir, simultaneamente, dados nas bandas X, C, S, L e P, gerando em torno de $\SI{20}{\frac{GB}{min}}$ %%% ACF Use siunitx
de dados.    

%%% ACF Sempre detalhe o significado de uma sigla como SAOCOM
Destacamos ainda o projeto SAOCOM pela atualidade e importância para o monitoramento da América Latina, informações podem ser obtidas no sítio web \url{https://directory.eoportal.org/web/eoportal/satellite-missions/s/saocom}. 
O satélite SAOCOM-1B foi colocado em órbita em 2020 conforme \url{https://www.space.com/spacex-saocom-1b-launch-rocket-landing-success.html}.

Como curiosidades, podemos citar: 
\begin{itemize}
\item O projeto de um mini-SAR para uso em veículos aéreos não tripulados que focam na evolução da micro-eletrônica para aumentar a eficiência e diminuir o peso; %%% ACF Referência ou link
\item As imagens SAR podem ser captadas por satélites, orbitando outros planetas, como o projeto Magellan SAR que orbita Vênus;  %%% ACF Referência ou link; orbita ainda?
\item Outras tecnologias usadas para a captação de imagens SAR denominada de interferométrica SAR (InSAR), que usa dois ou mais radares de abertura sintética para obter imagens. %%% ACF Referência ou link, e detalhe mais o que é o InSAR e para que serve
\end{itemize}
...
Entretanto, as desvantagens nos sistemas SAR e PolSAR não evitam as mesmas de serem largamente empregadas, o que impulsiona a produção de conhecimento nessa área, gerando um grande interesse, tanto em nível de aplicações, como em pesquisas científicas, como podemos ver 
na referência 
%%% ACF Não escreva "a referência"; diga se é artigo, livro, matéria de divulgação etc.
\cite{rijpbnm} ou no sítio 
