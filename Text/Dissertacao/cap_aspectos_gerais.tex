...
Neste capítulo são apresentadas algumas informações relevantes para situar e contextualizar a área de pesquisa desta tese. 
São abordados alguns assuntos introdutórios, traçando um breve panorama do desenvolvimento e aprimoramento dos radares RAR (\textit{Real Aperture Radar}),
SAR e PolSAR. 
...
Os símbolos hh, hv, vh, e vv representam as polarizações disponíveis nos radares, onde a primeira letra é a maneira como a onda é emitida e, a segunda letra é a maneira como a onda é recebida. Sendo h a polarização horizontal e v a polarização vertical.
\index{Polarização}
...
Importantes informações sobre radares podem ser encontradas no livro \citet{lp}.

O desenvolvimento dos radares pode ser resumido pelos princípios a seguir:
\begin{itemize}
...
No livro de \cite{lkc} encontramos um amplo espectro de aplicações das imagens SAR e PolSAR na área de sensoriamento remoto.  Listaremos aqui algumas dessas aplicações: 
...
A Tabela~\ref{tab:micro_onda_espectro} definida no livro \cite{lp} mostra na primeira coluna a representação da banda, na segunda coluna as faixas de frequências que os radares são construídos. Na terceira coluna foi fixada uma frequência (f) para encontrar o comprimento de onda ($\lambda$) e mostrado na quarta coluna. Lembrando que o produto da frequência pelo comprimento de onda determina a velocidade de propagação da mesma.   
...
%%% AAB referenciei a fonte da tabela e modifiquei
	\centering
	\caption{Espectro eletromagnético para a faixa de micro-ondas.}
...
\end{tabular}\label{tab:micro_onda_espectro}
\end{table}\index{Espectro Eletromagnético}
\index{Banda}
\index{Frequência}
\index{Comprimento de Onda}

A descoberta da tecnologia SAR impulsionou um grande desenvolvimento da área de sensoriamento remoto, levando à construção do SEASAT, o primeiro satélite SAR comercial projetado. 
Este projeto foi muito bem sucedido e estabeleceu definitivamente os sistemas SAR e PolSAR como área de pesquisa. 
\index{SEASAT}
\index{Características específicas}
...
Para mais informações consulte o seguinte sítio eletrônico, \url{https://earth.esa.int/web/eoportal/satellite-missions/s/seasat}.  
\begin{table}[hbt]
	\centering
...
\end{tabular}\label{tab:carac_seasat}
\end{table}

Outros projetos de sistema foram realizados e estão listados na Tabela \ref{tab:carac_radares}.
% Neste trabalho serão usadas imagens capturadas pelo radar AIRSAR, cuja características estão na Tabela~\ref{tab:carac_radares}.
%\begin{sidewaystable}
%\footnotesize
%	\centering
%	\caption{Características operacionais dos satélites SAR ou PolSAR.}
...
%\end{tabular}\label{tab:carac_radares}
%\end{sidewaystable}
\begin{table}[hbt]
    \tiny
	\centering
	\caption{Características operacionais dos sistemas SAR ou PolSAR.}
\begin{tabular}{@{}llllllll@{}} \toprule
Dispositivo & Nacionalidade&Lançamento&Banda&Polarização&Ângulo de incidência(\si{\degree}) &Distância(\si{\km})& 
Resolução(\si{\m\squared})\\ \midrule
SEASAT   &EUA             &\num{1978}&L    &hh      &\num{23}                             &\num{100}& \num{25}\\
AIRSAR   &EUA             &\num{1988}&P-L-C&hh-hv-vv&\numrange[range-phrase = --]{20}{60} &\numrange[range-phrase = --]{10}{17}&\numrange[range-phrase = --]{2}{8} \\
SIR--C    &Alemanha-Itália &\num{1990}&L-C-X&hh-hv-vv&\numrange[range-phrase = --]{15}{55} &\numrange[range-phrase = --]{15}{90}& \numrange[range-phrase = --]{10}{60}\\
ALMAZ    &Rússia          &\num{1992}&S    &hh      &\numrange[range-phrase = --]{30}{60} &\num{350}&\numrange[range-phrase = --]{10}{30} \\
ERS--2    &Europa          &\num{1995}&C    &vv      &\num{23}                             &\num{100}&\num{30} \\
JERS--1   &Japão           &\num{1998}&L    &hh      &\num{35}                             &\num{75}& \num{18}\\
RADSAT--1 &Canadá          &\num{1995}&C    &hh      &\numrange[range-phrase = --]{20}{59} &\numrange[range-phrase = --]{50}{500}& \numrange[range-phrase = --]{10}{100}\\
RADSAT--2 &Canadá          &\num{2003}&C    &hh-hv-vv&\numrange[range-phrase = --]{20}{60} &\numrange[range-phrase = --]{10}{500}& \numrange[range-phrase = --]{3}{100}\\\bottomrule
\end{tabular}\label{tab:carac_radares}
\end{table}
\index{Características Operacionais}
\index{AIRSAR}
\index{SIR--C}
\index{ALMAZ}
\index{ERS--2}
...
Para cada pixel fixo da imagem, os canais recebem os valores mostrados na Tabela~\ref{tab:canais}. Os símbolos $C_k$ com $k=1,\dots,9$ definem os canais da imagem.
...
Neste trabalho serão usados os três primeiros canais da matriz que armazena os dados da imagem PolSAR, conhecidos como canais de intensidades. Os sensores AIRSAR, SIR-C, e RADSAT-2 fornecem informações deste tipo, mais características desses sensores podem ser encontrados na Tabela \ref{tab:carac_radares}.
...
Um exemplo de dados PolSAR é a imagem da baía de São Francisco (EUA), com suas respectivas polarizações em tons de cinza, mostradas na figura~\ref{fig:sf_hh_hv_vv}. A imagem foi obtida pelo sensor aerotransportado AIRSAR polarimétrico na banda L. O AIRSAR apresenta resolução de \num{2} até \num{8} \si{\m\squared} por píxel.  Na Figura \ref{fig:sf_hh_hv_vv} o canal hh é mostrado a esquerda, no centro podemos visualizar o canal hv, enquanto a imagem da direita é o canal vv. 
...
\end{figure}
\index{São francisco -Imagem PolSAR}
...
A mesma região de São Francisco capturada por um satélite ótico através do sítio eletrônico \url{https://www.google.com/maps} é mostrada na Figura \ref{fig:otica_google}.
\begin{figure}[hbt!]
	\centering
...
\end{figure}
\index{São francisco -Imagem Ótica}

\index{VHR}
\index{Projeto Lynx}
\index{Sandia National Laboratories}
...
Um exemplo é o sistema Lynx, projetado pelo \textit{Sandia National Laboratories} localizado nos EUA , o qual alcança resoluções de $10$ a $30$ centímetros quadrados. O sítio eletrônico \url{https://www.sandia.gov/media/NewsRel/NR1999/Lynx.htm} fornece as informações da Tabela \ref{tab:carac_lynx} sobre o projeto Lynx:
...
Outro exemplo, pode ser visto no artigo \cite{rijpbnm}, que mostra um sistema PolSAR que produz imagens com resolução de \SI[product-units = brackets-power]{25 x 25}{\cm},
%%% ACF Use siunitx
%%% AAB realizado
...
de dados.    
\index{DLR}
\index{Projeto SAOCOM}
\index{Projeto Mini-SAR}
\index{Projeto Magellan SAR}

%%% ACF Sempre detalhe o significado de uma sigla como SAOCOM
%%% AAB Realizado
Destacamos ainda o projeto \textit{SAtélite Argentino de Observación COn Microondas} (SAOCOM) pela atualidade e importância para o monitoramento da América Latina, informações podem ser obtidas nos sítios web \url{http://saocom.invap.com.ar/} ou \url{https://directory.eoportal.org/web/eoportal/satellite-missions/s/saocom}. 
...
Entretanto, as desvantagens nos sistemas SAR e PolSAR não evitam os mesmos de serem largamente empregados, o que impulsiona a produção de conhecimento nessa área, gerando um grande interesse, tanto em nível de aplicações, como em pesquisas científicas.