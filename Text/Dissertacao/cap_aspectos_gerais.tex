...
Na referência \cite{lkc} encontramos um amplo espectro de aplicações das imagens SAR e PolSAR na área de sensoriamento remoto.  Listaremos aqui algumas dessas aplicações: 
...
O sistema PolSAR armazena informações de retorno em uma matriz $\mathbf{S}_{2\times 2}$ para cada ponto de sua região de observação, onde as entradas da matriz são números complexos. Se o sistema é mono-estático, isto é , a matriz torna-se hermitiana (a matriz é igual a sua transposta conjugada), por esse motivo, podemos representar a matriz na forma de um vetor $\mathbf{s}$ de dimensão 3. O produto entre o vetor e seu hermitiano gera uma matriz hermitiana $3\times 3$ com suas entradas mostradas na tabela \ref{tab:sistema_polsar}.
