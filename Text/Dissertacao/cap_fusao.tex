...
As evidências de bordas estão armazenadas em  $n_c$ imagens binárias $\{\widehat{\bm\jmath}_c\}_{1\leq c\leq n_c}$, onde o pixel de valor 1 denota uma estimativa de borda e o pixel de valor 0 denota um elemento, onde não foi detectado borda. As imagens (matrizes) têm tamanho  $m\times n$, onde podemos definir $\ell= mn$. Essas imagens foram usadas para a fusão resultando na imagem $I_\text{F}$ com as bordas detectadas.

Seis técnicas de fusão foram propostos:
\begin{itemize} 
\item Média simples; 
...
A figura~\eqref{fig:cap_fusao_media_simples} apresenta o fluxograma para a fusão por média simples, para obter mais detalhes conferir \citet{mit}.
...
O fluxograma para o método de fusão MR-DWT pode ser visto na figura \eqref{fig:cap_fusao_dwt}. Na figura abaixo W e $\text{W}^{-1}$ representam a transformação para obter a decomposição wavelet e a transformação inversa, respectivamente. 
\pgfdeclarelayer{background}
\pgfdeclarelayer{foreground}
\pgfsetlayers{background,main,foreground}
...
Este método de fusão é  baseado em~\cite{n_r, jjly}. A diferença entre os métodos MR-DWT e MR-SWT é a substituição do operador transformada discreta wavelet (DWT) pelo operador transformada estacionária wavelet (SWT).

O fluxograma para o método de fusão MR-SWT pode ser visto na figura \eqref{fig:cap_fusao_swt}. Na figura abaixo W e $\text{W}^{-1}$ representam a transformação para obter a decomposição wavelet e a transformação inversa, respectivamente.
\pgfdeclarelayer{background}
\pgfdeclarelayer{foreground}
\pgfsetlayers{background,main,foreground}
\tikzstyle{sensor}=[draw, fill=blue!20, text width=5.5em, 
...
O método PCA é baseada em~\cite{n_r,mit}, e pode ser resumido nos seguinte passos:
\begin{enumerate}
...
O método estatística \textit{Receiver Operating Characteristics} (E-ROC) descrito em~\cite{gs,fawcett} segue os seguintes passos:
\begin{enumerate}
...
Os valores da diagonal principal da matriz de confusão representam as classificações realizadas corretamente, enquanto os elementos da diagonal secundária, representam as classificações incorretas. A soma de todas as possibilidades dos resultados em uma classificação retorna o valor TP+FN+FP+TN=1.
...
Resumindo, somando-se as matrizes $\bm{\widehat\jmath}_c$ temos a matriz de frequência $\text{V}=\sum_{c=1}^{n_c}\bm{\widehat\jmath}_c$. Na matriz V é aplicado os limiares $\text{CT}_t$ com $t=1,\dots,n_c$, gerando os mapas de evidências de bordas $\bm{\widehat\imath}_t$. Na figura \eqref{fig:cap_fusao_roc1} pode-se visualizar o fluxograma correspondente a esse processo.
\pgfdeclarelayer{background}
\pgfdeclarelayer{foreground}
\pgfsetlayers{background,main,foreground}
\tikzstyle{sensor}=[draw, fill=blue!20, text width=5em, 
...
Para cada matriz $\bm{\widehat\imath}_t$, com $t$ fixado arbitrariamente, foram comparadas a todas as matrizes $\bm{\widehat\jmath}_c$, e foi construído para cada comparação uma matriz de confusão, desta forma gerou-se $n_c$ matrizes de confusão, mostrado na tabela \eqref{tab:matriz_conf_roc_j}.
\begin{table}[htb!]
	\centering
...
A referência~\cite{fawcett} mostra que o limiar ótimo ocorre na intersecção, ou perto da mesma, da curva ROC com a linha diagnóstico. Essa linha é formada conectando os pontos (P,P) e (0,1) representada pela linha vermelha na figura \eqref{fig:curva_roc}. 
...
O método fusão MR-SVD, veja~\citet{naidu}, trabalha de maneira similar ao MR-DWT. A diferença consiste em mudar os filtros DWT por filtros SVD. O método pode ser resumido nos seguinte passos: 
\begin{enumerate}
