...
Ver as referências \citet{mit, bmf_2020}. %%% ACF Aqui diga ao leitor o que ele deve "ver".
...
Um possível para resolver este problema é usar o método GenSA \citep[\textit{Generalized Simulated Annealing},][]{xgsh}.

Nesta pesquisa propomos, discutimos, e comparamos o uso de seis métodos de fusão: 
Média simples \citep{mit}; 
Transformada \textit{Wavelet} Discreta de Multi-resolução, MR-DWT \citep{n_r}; 
Análise de Componentes Principais, PCA \citep{n_r,mit}; 
Estatísticas baseadas na curva E-ROC \textit{Receiver Operating Characteristic} \citep{gs}; 
Transformada \textit{Wavelet} Estacionária de Multi-resolução, MR-SWT \citep{n_r, jjly}; 
e Decomposição em valores singulares multi-resolução, MR-SVD \citep{naidu,demmel}.
...
\begin{itemize}
	...
	\item[(ii)] Em cada ROI calcular o centro de massa e, a partir desse ponto, traçar retas radiais cruzando duas amostras distintas. 
	Nas radiais serão extraídas informações para o passo (iii);\index{Radiais}
	\item[(iii)] Nos dados extraídos em cada reta radial o método MLE é aplicado para estimar o ponto de transição entre duas amostras, chamado de evidência de borda;\index{Evidências de bordas}\index{MLE}
...
\item Realizar medidas de qualidade para a detecção em cada canal, e na fusão, usando uma métrica baseada na probabilidade de detecção de bordas, comparando com a imagem definida como referência (\textit{Ground Truth}).      
\item Identificação de duas características que dificultam a detecção de evidências de bordas: oscilação nas extremidades e não diferenciabilidade em vários pontos do domínio das funções de máxima verossimilhança total. 
...
A reprodutibilidade e replicabilidade são essenciais para o desenvolvimento da ciência, estes temas não são discutidos neste trabalho, porém realizamos as pesquisas de forma a respeitar esses conceitos.
...
\begin{enumerate}%%% ACF use \label e \ref sempre
