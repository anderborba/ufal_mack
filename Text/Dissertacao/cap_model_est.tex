...
\end{array}
\end{equation}
resultanto na função logarítmica da PDF~\eqref{pdf_gauss_univ} .
...
Sendo uma amostra $\bm z=(z_1,\dots,z_n)$ e aplicando o método da máxima verossimilhança,
\begin{equation}\nonumber
\begin{array}{ccc}
  \ell(\mu, L;\bm z)&=&\ln\prod_{k=1}^{n}f_Z(z_k;\mu,L)\\
  \ell(\mu, L;\bm z)&=&\sum_{k=1}^{n}\ln f_Z(z_k;\mu,L)
 \end{array}
 \end{equation}
na PDF~\eqref{func_log_univ_gaussiana}. teremos,
\begin{equation}\nonumber
\begin{array}{lcl}
    \ell(\mu, L;\bm z)&=&\sum_{k=1}^{n}\ln f_Z(z_k;\mu,L)\\
...
Definimos a equação reduzida log-verossimilhança para a PDF univariada $\Gamma$.
\begin{equation}\label{eq_log_vero_red_gauss}
    \ell(\mu, L;\bm z)=n\left[L\ln\frac{L}{\mu}-\ln\Gamma(L)\right]+(L-1)\sum_{k=1}^{n}\ln z_k - \frac{L}{\mu}\sum_{k=1}^{n} z_k.\\                
 \end{equation}
...
%\end{equation}
%onde é definido por $g=|\mathbf{S}_i\mathbf{S}_j^H|$ e $h=\sqrt{E[|\mathbf{S}_i|^2]E[|\mathbf{S}_i|^2]}$.
Considerando a função a PDF univariada magnitude do produto, 
\begin{equation}\label{pdf_mag_prod}
\begin{array}{lcl}
	f(\xi)&=&\frac{4L^{L+1}\xi^L}{\Gamma(L)(1-|\rho|^2)}I_0\left(\frac{2|\rho|L\xi}{1-|\rho|^2}\right)K_{L-1}\left(\frac{2L\xi}{1-|\rho|^2}\right).
		\end{array}
\end{equation}
onde, $\xi>0$ e $L>0$. Aplicando o logaritmo natural na equação~\eqref{pdf_mag_prod} e realizando algumas manipulações algébricas teremos:
\begin{equation}
\begin{array}{lcl}\nonumber
...
Definimos a equação reduzida log-verossimilhança para a PDF univariada magnitude do produto
\begin{equation}
\begin{array}{lcl}\label{eq_log_vero_red_mag_produto}
	\ln f(\xi)&=&\ln (4)+(L+1)\ln L+L\ln \xi-\ln\Gamma(L)-\ln(1-|\rho|^2)+\ln I_0\left(\frac{2|\rho|L\xi}{1-|\rho|^2}\right)+ \ln K_{L-1}\left(\frac{2L\xi}{1-|\rho|^2}\right).
		\end{array}
\end{equation}

\subsection{Método da verossimilhança aplicado na PDF univariada razão de intensidades}
%A razão de intensidade e amplitude entre $S_{hh}$ e $S_{vv}$ são importantes no estudo de radares polarimétricos. A $PDF's$ razão de intensidade e amplitude normalizada será mostrada agora
...
%A $PDF$ razão intensidade {\it multlook} normalizada é mostrada no apêndice $(C)$ do artigo \cite{lee}  
Considerando a função a PDF univariada magnitude do produto,
\begin{equation}\label{pdf_razao_intensidades}
	p^{(L)}(\mu)=\frac{\Gamma(2L)(1-|\rho_c|^2)^{L}(1+\mu)\mu^{L-1}}{\Gamma(L)\Gamma(L)\left[(1+\mu)^2-4|\rho_c|^2\mu \right]^{\frac{2L+1}{2}}}\\
\end{equation}
onde, $\mu>0$ e $L>0$. Aplicando o logaritmo natural na equação~\eqref{pdf_razao_intensidades} e realizando algumas manipulações algébricas teremos:
\begin{equation}\nonumber
\begin{array}{lcl}
...
Definimos a equação reduzida log-verossimilhança para a PDF univariada razão de intensidades

\begin{equation}\label{eq_log_vero_red_razao_intensidade}
\begin{array}{lcl}	
	\ln p^{(L)}(\mu)&=&\ln\Gamma(2L) +L\ln(1-|\rho_c|^2)+\ln(1+\mu)+(L-1)\ln\mu\\
	&-&2\ln\Gamma(L)-\frac{2L+1}{2}\ln\left[(1+\mu)^2-4|\rho_c|^2\mu \right].\\
\end{array}
