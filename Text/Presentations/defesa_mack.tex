...
\begin{tabular}{@{}llr@{}} \toprule
	...
	Resolução    & \SI[product-units = brackets-power]{8 x 8}{\m\squared} \\ \bottomrule
\end{tabular}
\end{table}
\end{alertblock}
\end{frame}
...
\begin{description}
      ...
      \item[-] Ruído multiplicativo $\mathbf{Y}=X\cdot \mathbf{Z}$, o ruído $\mathbf{Z}$ é o \textit{Speckle}.
\end{description}
\end{itemize}
\end{alertblock}
\end{frame}
...
%
\begin{frame}[fragile]{Modelagem estatística}
\begin{alertblock}{Função máxima verossimilhança total}
\begin{figure}[hbt!]
	\centering
     \subfloat[Canal $\mathbf{I_\text{hh}}$]{%
     ...
     \subfloat[Canal  $\mathbf{I_\text{hv}}$]{%
     ...
     \subfloat[Canal  $\mathbf{I_\text{vv}}$]{%
     ...
     \caption{Funções de verossimilhanças totais para cada canal}
